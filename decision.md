# Jack's College Decision — Framing & Comparison

*For Jack and his parent to review together. Companion to `background.md`, which contains the full research base.*

---

## What we're deciding

Jack wants to be in a **competitive rowing program with a real rowing culture** — being in a B or C boat at a serious program is fine — **and** he wants a Bachelor of Engineering. Those two goals pull in different directions, and most of his viable paths stack conditional hurdles — 3:2 transfer admits, GPA thresholds, rowing eligibility — on top of each other. The question is: **which starting school gives him the best shot at a BS in engineering without sacrificing the rowing culture he came for, and at what cost and risk?**

---

## The decision framework

### This is a 3-way starting-school choice

The real decision is **where Jack shows up as a freshman.** Everything else — dual-degree transfer, fallback, military entry — is a sub-branch off that first choice.

1. **WPI** — Strong engineering, low risk, mid-competitive DIII rowing. Simple: focus on education and rowing, done in 4 years. Cheapest path (~$144K). **Trade:** rowing peaks at mid-high pack DIII.
2. **Hobart** — Solid IRA-system rowing with a path to **both** elite rowing (only 1 year at Columbia or Dartmouth LW) **and/or** solid-to-elite engineering (Columbia, Dartmouth, WashU BS+MS, RPI, or Cal State). Multiple fallbacks at the junior-year fork — see [`hobart.md`](./hobart.md) for the per-option detail. **Trade:** 5–6 years and ~$154K–$242K depending on which fork, plus a GPA gate sophomore/junior year. Worst case if every dual-degree fork fails: Hobart BA + Cal State BSME at ~$154K over 6 years — still a viable landing spot.
3. **UBC** — Solid engineering (QS #25 globally, #2 in Canada), elite Canadian varsity rowing **if** he makes the team, and an amazing lifestyle / location. Predictable 4-year degree at ~$193K with no transfer gate. **Trade:** Jack's own assessment is that walking on is uncertain, and the LW roster is ~4 slots. The realistic base case is no rowing at UBC. Treat it as "engineering + lifestyle, rowing optional."

**A military option can attach to any of the three paths** — applying to USNA this summer as a wildcard, or post-grad OCS/NUPOC after any of them. It doesn't compete with the starting-school choice, so the 3-way framing stands. See [Military option](#military-option) below for the short version, or [`military.md`](./military.md) for role types, entry paths, and the GI Bill + Yellow Ribbon cost math (Option 2f is the specific Hobart + OCS instance).

### Which fits which priority


| Priority you weight highest                                              | Starting school |
| ------------------------------------------------------------------------ | --------------- |
| Engineering certainty, cost, simplicity, done in 4 years                 | **WPI**         |
| Rowing ambition (esp. Ivy LW) with engineering upside, done in 5-6 years | **Hobart**      |
| Engineering-first with lifestyle and *possible* elite row                | **UBC**         |

---

# Reference material

*Everything below is the evidence the 3-way choice is built on: the full option set with its Hobart sub-branches, the side-by-side comparison, cost assumptions, rowing eligibility detail, and the military option detail. Read what's relevant; the decision itself is above.*

---

## Option set

Primary paths under consideration. Options 2a–2f all start at Hobart and differ at the junior-year fork; see [`hobart.md`](./hobart.md) for detail.


| #   | Option                            | One-line description                                                          |
| --- | --------------------------------- | ----------------------------------------------------------------------------- |
| 1   | **WPI**                           | 4-year ABET BS Engineering, done                                              |
| 2a  | **Hobart → Columbia (3:2)**       | Hobart BA (3 yrs) + Columbia SEAS BS (2 yrs); apply junior year               |
| 2b  | **Hobart → Dartmouth (2:1:1:1)**  | Hobart → Dartmouth Y3 → Hobart Y4 → Dartmouth Y5 BE; apply sophomore year     |
| 2c  | **Hobart → WashU (3:3)**          | Hobart BA + WashU BS + MS; apply junior year                                  |
| 2d  | **Hobart → RPI (3:2)**            | Hobart BA + RPI BS; apply junior year                                         |
| 2e  | **Hobart BA → State School**      | Hobart BA in physics, then apply to a Cal State for BS                        |
| 2f  | **Hobart BA → OCS → MS**          | Hobart BA → commission via OCS → free MS at Dartmouth / Cornell via GI Bill   |
| 3   | **UBC**                           | 4-year BASc Engineering in Canada                                             |


---

## Comparison table

Rating scale: **Excellent / Strong / Solid / Modest / Weak**. Risk is rated as **Low / Moderate / High / Very High** for achieving a BS in engineering.

**Rowing priority:** Jack wants to be in a **competitive rowing program with a real rowing culture.** Being in a B or C boat at a serious program is fine; being in the top boat at a recreational club is not.

**Rowing Culture Fit** in the table below is a one-line summary. See [Rowing fit, made explicit](#rowing-fit-made-explicit) and [Critical eligibility note](#critical-eligibility-note-why-5-years-at-the-ivy-does-not-mean-2-years-of-ivy-rowing) for the detail on IRA/Ivy eligibility and why both Ivy paths deliver only 1 year of Ivy LW racing.


| Option                               | Engineering Reputation                                                                                             | Rowing Culture Fit                                                                                                                                                                                                                                        | Tuition Cost                                | Risk                                                                            |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------- |
| **1. WPI**                           | **Strong.**<br>#64 ranked engineering. Engineering from day one. Project-based curriculum.                            | **Solid, mid-pack DIII.**<br>DIII IRA system (no NEWMAC men's championship); New England regional circuit. Not selected to IRA DIII 2025; finished 2nd in V8 3rd-level final at NIRC. [See below.](#rowing-fit-made-explicit)                                | **~$144K / 4 yrs.**<br>BS Engineering.         | **Very Low.**<br>Engineering from day one.                                         |
| **2a. Hobart → Columbia (3:2)**      | **Excellent.**<br>Ivy League. #21 ranked engineering.                                                                 | **Elite but limited: 1 yr of Columbia LW.**<br>3 Hobart IRA seasons + 1 Columbia LW (Y4 only; Y5 Ivy-ineligible as grad). [See below.](#rowing-fit-made-explicit)                                                                                            | **~$242K / 5 yrs.**<br>BA + Columbia BS.       | **High.**<br>3.50 GPA gate, junior-year fork, competitive review (not guaranteed). |
| **2b. Hobart → Dartmouth (2:1:1:1)** | **Excellent.**<br>Ivy League. #51 ranked engineering.                                                                 | **Elite but limited: 1 yr of Dartmouth LW.**<br>2 Hobart IRA + 1 Dartmouth LW (Y3) + optional 1 Hobart openweight (Y4); Y5 Ivy-ineligible as grad. [See below.](#rowing-fit-made-explicit)                                                                   | **~$242K / 5 yrs.**<br>BA + Dartmouth BE.      | **High.**<br>~3.30 GPA gate, sophomore-year fork, competitive review.              |
| **2c. Hobart → WashU (3:3)**         | **Strong.**<br>#42 ranked engineering.                                                                                | **Nationally competitive club.**<br>3 Hobart IRA + 1 WashU ACRA (ACRA's 4-in-5 clock is binding). WashU M4+ 2024 ACRA national champion, M2- 2025 bronze — top ACRA programs (UVA, UCLA, Purdue) rival DIII varsity. [See below.](#rowing-fit-made-explicit) | **~$205K / 6 yrs.**<br>BA + BS + MS.           | **Low.**<br>~90% admit at 3.25 GPA with prereqs done — near-automatic.             |
| **2d. Hobart → RPI (3:2)**           | **Strong.**<br>#35 ranked engineering.                                                                                | **Strong. Ends at Hobart.**<br>3 Hobart IRA + rec-level RPI crew (no ACRA/IRA footprint). [See below.](#rowing-fit-made-explicit)                                                                                                                            | **~$236K / 5 yrs.**<br>BA + RPI BS.            | **Low.**<br>Near-automatic at ~3.00 GPA.                                           |
| **2e. Hobart BA → State School**     | **Solid.**<br>Engineering from a Cal State.                                                                           | **Strong. Ends at Hobart.**<br>4 Hobart IRA + club-level Cal State. [See below.](#rowing-fit-made-explicit)                                                                                                                                                  | **~$154K / 6 yrs.**<br>BA + CSU BS.          | **Low–Moderate.**<br>CSU typically needs 2.7 GPA and prereqs done.                 |
| **2f. Hobart BA → OCS → MS**         | **Solid to Elite.**<br>Hobart BA solid; tuition-free MS at Dartmouth Thayer / Cornell / Stanford via GI Bill + Yellow Ribbon post-service lands the credential elite.                                                                            | **Strong. Ends at Hobart.**<br>4 Hobart IRA + no college rowing after. [See below.](#rowing-fit-made-explicit)                                                                                                                                                       | **~$137K undergrad + $0 grad.**<br>BA + elite MS after service. | **Moderate.**<br>OCS selection competitive but viable; 3–5 yr service commitment is the real cost. |
| **3. UBC**                           | **Strong. Solid in US hiring contexts.**<br>QS #25 globally, #2 in Canada. Lower brand recognition with US employers. | **Elite — walk-on risk.**<br>Elite program, but Jack not confident he'd make the team; 4 LW slots. Base case: no rowing at UBC. [See below.](#rowing-fit-made-explicit)                                                                                      | **~$193K (USD) / 4 yrs.**<br>BASc Engineering. | **Low.**                                                                        |


**Add-on to Option 1 (WPI):** If Jack lands at WPI and posts a strong record (3.8+ GPA, solid extracurricular profile), he *can* apply as an external transfer to Columbia / Cornell / Dartmouth engineering after year 1 or 2 — but this is a **highly competitive long shot** (Columbia external transfer ~5–6%, Cornell Engineering ~10%), structurally worse odds than the 3:2 pipeline at Hobart, and it forfeits IRA rowing eligibility seasons on re-walk-on. Treat it as an opportunistic upside, not a primary path.

### Cost assumptions used in the table

- **Hobart:** 50% merit scholarship → **~$34K/yr net tuition** (from $68,414 sticker). **Needs to be confirmed in the offer letter.**
- **WPI:** $27K/yr merit scholarship already awarded → **~$36K/yr net tuition** (from $63,046 sticker).
- **Columbia / Dartmouth (Ivy):** **full sticker, no aid assumed** (~$70K/yr).
- **WashU (graduate years of the 3:3):** automatic **50% Graduate Affiliation Scholarship** on graduate tuition → **~$34.5K/yr net** (from $69,190 sticker). Structural feature of the 3:3 program, not a need-based award.
- **RPI, UBC:** full sticker.
- **SDSU (and other Cal States):** **California resident tuition + fees ~$9K/yr.** Jack is a California resident. WSU is out-of-state (~$30K/yr non-resident) and is dropped from the cost model in favor of SDSU.

### Critical eligibility note: why the 5- and 6-year paths do NOT mean 5–6 years of racing

Three separate eligibility systems apply across Jack's option set, and **all three cap competition at 4 seasons completed within 5 calendar years of first full-time collegiate enrollment:**

1. **IRA** (governs Hobart, Columbia, Dartmouth men's LW). Adopts IRA rules as baseline: 4 seasons in 5 years, freshman participation counts as a season used.
2. **Ivy League conference overlay** (Columbia, Dartmouth — NOT Hobart):
  - **Graduate students are ineligible for varsity athletics.** A one-time COVID waiver in 2021–22 was explicitly not renewed. Columbia's 2025–26 LW roster confirms this — no grad students.
  - **No redshirts.** If enrolled during the season, it counts whether he races or not.
3. **ACRA** (governs WashU, club rowing). Its own written 4-in-5 clock: "four years of eligibility, which must be completed during the first 10 semesters or 15 quarters after the student is enrolled in a collegiate institution." Prior IRA/NCAA seasons at another school **count against the ACRA clock** — ACRA Rule #2 says "competing for an institution in any one event within the academic year is enough to constitute one year of eligibility." ACRA does allow grad students to race *if* they have eligibility remaining and are within the 5-year window (Rule #5), but you can only row for 4 years.

**Practical implication for each path:**

- **Columbia 3:2 — 1 year of Columbia LW racing (Y4).**  
Hobart Y1–3 burns 3 of 4 IRA seasons. Y4 at Columbia is senior undergrad — eligible. Y5 at Columbia is graduate standing — **Ivy-ineligible**, and Ivies don't allow redshirting Y4 to preserve for Y5.
- **Dartmouth 2:1:1:1 — 1 year of Dartmouth LW racing (Y3).**  
Hobart Y1–2 burns 2 seasons. Y3 at Dartmouth as junior undergrad — eligible. Y4 back at Hobart — eligible, but openweight only. Y5 at Dartmouth is graduate standing — **Ivy-ineligible**.
- **WashU 3:3 — 1 year of WashU ACRA racing (Y4)**  
Hobart Y1–3 burns 3 of 4 ACRA seasons. Y4 at WashU uses his 4th and final ACRA season AND is clock year 4 of 5. Y5 at WashU (grad year 1): 0 seasons remaining under ACRA Rule #2, and clock year 5 is the last. Y6 is outside the 5-year clock entirely.

---

## Military option

There are military options, and they attach to any of the primary paths rather than replacing the starting-school choice. Two matter for the decision above:

- **USNA this summer (wildcard that preempts everything above).** If Jack secures a Congressional nomination and gets admitted, USNA changes the game — tuition-free ABET engineering BS, DI lightweight rowing at Eastern Sprints, 5-year service commitment. **Important disclaimer: USNA does not accept transfer credit.** If admitted after his Hobart freshman year, Jack **restarts as a plebe** and graduates Class of 2031 instead of 2030. Admit rate ~8% plus the nomination hurdle, so low-probability, but if it lands it preempts the 3-way decision. Worth applying in parallel; rejection doesn't close any door.
- **OCS after graduation (if military service becomes an aspiration).** Any bachelor's degree qualifies for unrestricted-line OCS (SWO, aviation, NFO, intel, supply). An ABET engineering BS additionally qualifies for engineering-designator OCS (EDO, CEC). This route is available after any primary path — no decision needed now, no action during college.

**For role-type detail, NUPOC (junior/senior year Navy nuclear), the full entry-path breakdown, and per-primary-path interaction table, see [`military.md`](./military.md).** For the specific Hobart BA → OCS → tuition-free MS cost mechanics (GI Bill + Yellow Ribbon), see [Option 2f in `hobart.md`](./hobart.md#the-other-hobart-forks).

---

*See `background.md` for the full research base, cost breakdowns, eligibility details, and specific action items.*
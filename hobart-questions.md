...
## 1. Prof. Donald Spector — HWS engineering liaison

Primary HWS contact for every Hobart path. Ideal conversation timing is **post-July 2026** once the May 2026 Physics C and Micro scores have posted. Full AP detail lives in [`ap-credits.md`](./ap-credits.md); goal-attribute and term-offering detail lives in [`hobart-plan.md`](./hobart-plan.md#-items-to-confirm-with-prof-spector-before-matriculation) and [`hobart-electives.md`](./hobart-electives.md).

| # | Priority | Question | Why it matters |
|---|----------|----------|----------------|
| 1.1 | **P0** | Does a 5 on **AP Physics C: Mech and E&M** buy **exemption** from PHYS 150 / 160 (not just credit)? Calc-based Physics C matches HWS's calc-based sequence, which strengthens the exemption case. | Single most schedule-shaping question. If yes, triggers the [Optimistic variant in `ap-credits.md`](./ap-credits.md#optimistic-variant--if-prof-spector-accepts-ap-physics-for-phys-150160) and opens up 2 additional elective slots in Y1. |
...
| 2.2 | **P1** | **AP-credit posting order** when the pile is over the 7-unit cap. Coordinate with Spector §1.3 ruling. | Determines which 7 appear on the transcript and which don't. |
...
| # | Priority | Question | Why it matters |
|---|----------|----------|----------------|
| 4.1 | **P1** | **Y3 non-engineering course mix.** Confirm that the 3 non-engineering slots at Dartmouth Y3 can be chosen to carry HWS goal attributes back. Pair with Spector §1.7. | Drives whether 2b actually delivers the "STEM-heavier at Hobart" profile advertised in [`README.md`](./README.md#curriculum-fit). |
...
### Email — Prof. Spector (covers §1.1, §1.3, §1.4, §1.9)

Top-3 P0/P1 questions packaged for first contact. Sent before mid-July 2026 with a follow-up call planned once Physics C scores post.

> **To:** `spector@hws.edu`
> **Subject:** Dual-degree + AP questions for prospective fall '26 student
>
> Hi Prof. Spector,
>
> My son Jack is looking at Hobart for fall 2026 and is interested in the engineering dual-degree route. He's mainly weighing the Columbia 3:2 and Dartmouth 2:1:1:1, with the Syracuse ECS 4+2 on the radar as a backup MS path. Three things I'm hoping you can help us think through:
>
> 1. **Any engineering partners beyond Columbia, Dartmouth, and Syracuse?** I've seen scattered references to an RPI 3+2 and a WashU 3:3, but neither shows up in the HWS catalogue's Joint Degree Programs section. Could you share the current list of formally articulated partners? And if RPI or WashU aren't on it, is there a standard-transfer pathway with course-articulation support you've seen work for HWS students?
>
> 2. **AP Physics C placement.** Jack sits Physics C: Mechanics and E&M on May 13–14 (scores mid-July). Assuming he pulls 5s, does that get him (a) credit only, (b) credit plus exemption from PHYS 150/160, or (c) neither? Since Physics C is calc-based, exemption would be ideal — it'd let him jump straight into the upper-level physics sequence in Y1.
>
> 3. **AP cap and stacking priority.** Jack's pile is big: Calc BC, CS A, CS Principles, US History, Physics 1, Physics 2 — all 5s — plus Physics C Mech/E&M and Micro pending in May. That's potentially ~14 course units against the HWS 7-course cap. When the pile runs over: do Physics 1/2 and Physics C stack or only one set count? Which credits get priority? And do AP credits count toward the 32-course BA total (matters if Jack ends up self-completing the BA at Hobart over four years)?
>
> Happy to set up a quick call after mid-July once the Physics C scores are in and we can talk about real numbers — but a short reply or email thread works great too.
>
> Thanks,
> Gary [Last name]

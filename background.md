# Background — Jack's College Decision

*Longform reference document. All specifics pulled from prior agent-conversation research plus additional research on the military-option wildcard and state-school engineering transfer pathways. Companion to:*

- *[`README.md`](./README.md) — the primary decision doc with the 3-way framework (WPI / Hobart / UBC), comparison table, and short summaries.*
- *[`hobart.md`](./hobart.md) — read only if the starting-school choice is Hobart. Detailed per-option breakdown of the junior-year fork (2a–2g), portfolio strategy, eligibility reframing, and the Option 2g GI Bill + Yellow Ribbon mechanics.*
- *[`military.md`](./military.md) — read only if a military path is under serious consideration. Role-type table, entry paths (USNA / NUPOC / OCS / Army Direct Commissioning), per-primary-path interaction table, and practical recommendations.*

*Research date: April 2026. Rankings, costs, and rowing results should be re-verified before any commitment.*

---

## 1. The student

**Jack** — high school senior, rower (lightweight), interested in engineering (aerospace or mechanical preferred).

### AP scores on record

| Exam | Score |
|---|---|
| Calculus AB | 5 |
| Calculus BC | 5 |
| Physics 1 | 5 |
| Physics 2 | 5 |
| Computer Science A | 4 |
| Computer Science Principles | 4 |
| US History | 4 |
| Macroeconomics | 3 |

### Currently taking (scores pending summer 2026)

- AP Physics C: Mechanics
- AP Physics C: Electricity & Magnetism

### Why the Physics C scores matter

AP Physics 1 and Physics 2 are **algebra-based** and do **not** satisfy Columbia's calc-based physics prerequisite. Only AP Physics C does. If both C scores come through as 4s or 5s, Jack has only **5 remaining courses** to complete at Hobart for Columbia 3:2 prerequisites (Calc 3, Diff Eq, Gen Chem, Econ, University Writing). That makes the 3-year Hobart timeline realistic. If the Physics C scores come back as 3s or lower, the prereq load gets heavier and the 3:2 timeline gets tighter.

---

## 2. Stated preferences (from prior handoff)

- **BS in engineering is the priority.** An MS is acceptable if it's a structural feature of the program (e.g., WashU's 3-3) but not a target in itself.
- **Wants to start at Hobart if feasible.**
- **Strong bias toward elite rowing**, particularly Ivy lightweight (Columbia, Dartmouth).
- **Primary plan as stated:** Hobart physics + Columbia 3:2 engineering dual degree.

---

## 3. The schools — full profiles

> **Reframing note (April 2026, updated 2026-04-24):** This section is the **historical research base** and is preserved as written. The active option set has since narrowed: §3d (WashU McKelvey via 3:3) and §3e (RPI via 3:2) are now **definitively closed** — Prof. Spector's 2026-04-24 email confirmed "no, there is not a joint degree relationship with Wash U. or Rensselaer." Both paths are archived in [`hobart-unconfirmed-paths.md`](./hobart-unconfirmed-paths.md) as closed (the reactivation pathway has been retired). See the answered-blockquote in [`hobart-questions.md` §1](./hobart-questions.md#1-prof-ileana-dumitriu--hws-engineering-liaison) for the verbatim Spector quote. §3j (Syracuse 4+2) has been **promoted from "backup-to-the-backup" to a third primary dual-degree / direct-graduate-admission path** alongside Columbia 2a and Dartmouth 2b. Decision-grade summaries live in [`README.md`](./README.md) and [`hobart.md`](./hobart.md); the prose below each affected subsection (3d, 3e, 3j) has been re-headed to flag this status, but the underlying research is otherwise unedited.

### 3a. Hobart and William Smith Colleges (HWS)

- **Location:** Geneva, NY
- **Size:** ~2,200 undergrads across Hobart (men) and William Smith (women)
- **Role in Jack's plan:** Primary undergraduate home for years 1–3 (or 1–4 if Applied Physics only). BA in Applied Physics with pre-engineering coursework.

#### Hobart rowing

- NCAA DIII school, but men's rowing **operates in the IRA system**, making it effectively Division I-level for rowing purposes.
- **Ranked #24 in the 2025 D1/D2 IRA coaches poll.**
- **Major development — August 2025:** Hobart announced that starting with the 2026 season, they will compete at the **Dad Vail Regatta** instead of the NIRC. The Dad Vail offers 4 automatic qualifier spots to IRA Nationals. Head Coach Chris Kerber: *"The pathway to the National Championship now runs through the Dad Vail Regatta."* Meaningful upgrade in competition level.
- **No dedicated lightweight category** — openweight only.
- Head coach: Chris Kerber.

#### Hobart cost

- Tuition: $68,414/yr (2025–26)
- Room & board: $18,668/yr (not used in this analysis — tuition-only comparison)
- **Merit scholarship: $41,000/yr** (~60% off the $68,414 sticker) → **~$27,414/yr net tuition**. Working assumption throughout this document and `README.md`. **Applies only in years Jack is enrolled at Hobart** — the award comes off Hobart's tuition bill, it doesn't follow him to a 3:2 / 3:3 partner school. So it counts 3 years for 2a/2c/2d, 3 years for 2b (Y1/Y2/Y4 at Hobart; Y3/Y5 at Dartmouth), and 4 years for 2e/2f/2g. **Renewal terms still need offer-letter confirmation** — specifically the GPA minimum, semester cap, and how a formal leave of absence for the Dartmouth Y3 interacts with renewal (relevant only for Option 2b).
- AP credit: Hobart grants credit for AP scores of 4 or 5, maximum 7 course units total.

#### Engineering liaison at Hobart

**Prof. Ileana Dumitriu** (`dumitriu@hws.edu`) per Prof. Donald Spector's 2026-04-24 email redirect. Spector (`spector@hws.edu`, physics) was our first contact and answered the two paths-status questions (§1.1, §1.9 in [`hobart-questions.md`](./hobart-questions.md)) before routing further engineering questions to Dumitriu. The "four 3:2 partner agreements" framing in prior versions of this bullet was wrong: Spector's reply confirmed **no WashU or RPI partnerships exist** — the canonical engineering set is Columbia (3:2), Dartmouth (2:1:1:1), and the Syracuse 4+2 direct-MS path. Physics placement (AP Physics C exemption question) routes separately to **Prof. Ted Allen**, physics dept chair (`allen@hws.edu`), via an August-orientation in-person meeting.

---

### 3b. Columbia University (via 3:2)

- **Role in Jack's plan:** Years 4–5 of the 3:2 Combined Plan, earning the BS in Engineering.
- **Degree outcome:** BA from Hobart + BS in engineering from Columbia.
- **Columbia engineering ranking:** ~#18 nationally (US News graduate engineering).

#### Columbia 3:2 admission gate

- **Minimum 3.50 overall GPA** for affiliates with agreements signed in 2017 or later.
- **Admission is competitive review — NOT guaranteed** for anyone who began college Fall 2019 or later. This changed in 2019; older handoff language saying "guaranteed if GPA met" is stale.
- Pre-engineering GPA ≥ 3.30 required.
- Minimum grade of B on first attempt in all science and math prerequisites.
- Junior-year application, February deadline; decisions by June.

#### Columbia 3:2 required prerequisites

1. Full calculus sequence through Calc IV (multivariable)
2. Ordinary Differential Equations
3. Two semesters of calculus-based physics
4. General Chemistry I
5. Chemistry or Physics Lab
6. One semester of programming (C, Java, or MATLAB)
7. Principles of Economics
8. University Writing
9. 27+ nontechnical credits

#### What Jack's AP scores cover

| Prerequisite | AP coverage | Still needed? |
|---|---|---|
| Calc I | Calc AB (5) | Covered |
| Calc II | Calc BC (5) | Covered |
| Calc III (multivariable) | None | **Yes** |
| Calc IV / Diff Eq | None | **Yes** |
| Physics I (calc-based) | Physics C Mech (assumed 5 — May 2026) | Covered (pending confirmation) |
| Physics II (calc-based) | Physics C E&M (assumed 5 — May 2026) | Covered (pending confirmation) |
| General Chemistry | None | **Yes** |
| Computer Science | CS A (4) | Covered |
| Economics | Macro (3) — below credit threshold | **Yes** |
| University Writing | None | **Yes** |

If Physics C scores come in as 4s or 5s, that's 5 remaining required courses at Hobart.

#### Columbia rowing — lightweight

- **Elite Ivy League program** for lightweight specifically.
- **#1 nationally in 2023** IRCA/IRA Coaches Poll.
- **Won lightweight V8 national title in 2022.**
- Dedicated lightweight V8, 2V8, and 4+ boats.
- Races at Eastern Sprints and IRA Nationals.
- Head coach (lightweight): Nich Lee Parker.
- **Eligibility constraint:** Under the 3:2 path, Jack arrives at Columbia having burned 3 of 4 IRA seasons at Hobart. Y4 is senior undergrad and rowing-eligible; **Y5 is graduate standing and Ivy-ineligible under Ivy League conference policy** (the Ivy overlay trumps IRA here), and Ivy policy also bars redshirts. Result: **1 year of Columbia LW racing (Y4)**, not the previously planned Y4 redshirt + Y5 race pattern. See Section 4 for full rule stack.
- Elite program structurally, but eligibility rules cap Jack's Columbia LW racing at a single season.

#### Columbia cost

- Tuition: $70,170/yr (full sticker assumed — no aid)
- NYC living: ~$20–25K/yr (not included in tuition-only analysis)

---

### 3c. Dartmouth (via 2:1:1:1)

- **Role in Jack's plan:** Alternative 3:2 (technically 2:1:1:1) — 2 years Hobart, year 3 Dartmouth, year 4 Hobart (earning BA), year 5 Dartmouth (earning BE).
- **Degree outcome:** BA from Hobart + BE from Dartmouth Thayer.

#### Dartmouth 2:1:1:1 admission

- Application deadline **February 1 of sophomore year** — much earlier than 3:2 programs.
- Decisions arrive mid-April of sophomore year.
- Required GPA ~3.30; very competitive.
- **Two-step admission:**
  - Sophomore-year acceptance admits him to the Partner School Dual-Degree Program (covers year 3 exchange at Dartmouth). Real competitive admission.
  - **The BE year 5 requires a separate application to Thayer** (deadline Dec 15 of senior year). Returning Dual-Degree students are automatically admitted provided they apply on time and stay in good standing.

#### Performance risk in year 3 at Dartmouth

Dartmouth's quarter system (10-week terms) with six ENGS courses in year 3 is demanding.
- **Bs and Cs:** BE admission still administrative. Transcript is mediocre but nothing blocks.
- **Ds or an E (Academic Risk/Warning):** Thayer may exercise discretion. Conversation with Dual-Degree director required.
- **Suspension/Separation:** BE off the table. Off-ramp becomes finishing Hobart BA + standalone transfer to another engineering school (~2 more years).

**Critical caveat:** The Dartmouth transcript is visible on ALL downstream fallback applications. Unlike the 3-2 path (where only Hobart grades are evaluated), the Dartmouth 2:1:1:1 puts high-stakes engineering grades on his permanent record *before* the fallback decision happens. Poor Dartmouth performance damages every plan B.

#### Dartmouth rowing

- **Elite men's lightweight program — currently at the top tier of Ivy lightweight racing.**
- **2025 IRA National Championship: silver medalist.** V8 finished 5:32.250, under three seconds behind national champion Harvard. Team finished as national runners-up with 45 team points — head coach called it "the highest finish in program history."
- **2025 Eastern Sprints:** V4 won silver (7:00.700, narrowly behind Harvard); V8 finished fifth in grand final. First lightweight medal for Dartmouth at Eastern Sprints in over a decade.
- Races in the Ivy lightweight circuit vs. Harvard, Yale, Cornell, Columbia, Princeton.
- 2025–26 season schedule includes National Collegiate Lightweight Invitational (March 28, 2026), Eastern Sprints (May 17, 2026 in Worcester, MA), and IRA National Championship (May 29–31, 2026 in Sacramento, CA).
- Head coach: **Trevor Michelson.**
- **Strong fit for lightweight Jack, but constrained by Ivy conference policy.** Under the 2:1:1:1 structure Jack would row Dartmouth LW **in Y3 only** (1 season). Y4 at Hobart would be openweight or no-race. **Y5 at Dartmouth is graduate standing, and Ivy League conference policy bars graduate students from varsity athletics** — so Y5 LW racing at Dartmouth is not available. IRA alone would not block Y5; the binding constraint is the Ivy overlay. See Section 4 (IRA and Ivy League rowing eligibility) for full rule stack. The previously envisioned "race Y3 + Y5 at Dartmouth" pattern is not achievable under current Ivy policy.

#### Dartmouth cost

- Tuition at Thayer comparable to Columbia (~$70K/yr, full sticker assumed).

---

### 3d. WashU McKelvey (via 3:3)

> **⚠ Status (updated 2026-04-24): CLOSED — no HWS partnership.** Spector's 2026-04-24 email confirmed no joint-degree relationship exists with WashU. HWS's 2024–25 catalogue Joint Degree Programs page lists only Columbia and Dartmouth; WashU McKelvey's affiliate list does not include HWS. The deep research below pre-dates both findings — kept here as the historical research base, but Path 2c is dropped from the active option set. Closed-path archive lives in [`hobart-unconfirmed-paths.md`](./hobart-unconfirmed-paths.md); verbatim Spector quote in [`hobart-questions.md` §1](./hobart-questions.md#1-prof-ileana-dumitriu--hws-engineering-liaison).

- **Role in Jack's plan (if confirmed):** Fallback partner for 3:3 Dual Degree. **This was modeled as the strongest fallback option** before partnership-status review demoted it.
- **Degree outcome:** BA from Hobart + **BS + MS** in engineering from WashU McKelvey. Six total years.

#### WashU 3:3 admission

- **Minimum 3.25 GPA** (overall and math/science).
- Per WashU's FAQ: **all applicants are admitted if they meet the 3.25 GPA, complete prereq courses, and get the institutional endorsement.** ~90% admit rate.
- Barring a felony or academic integrity violation, admission is effectively automatic.

#### WashU economics (most attractive on cost)

- Automatic **Graduate Affiliation Scholarship**: flat **50% tuition discount** for all 3 years at WashU (structural feature of the program, not need-based).
- At 2026-27 rates ($69,190/year graduate tuition), that's **~$34,500/year post-discount × 3 years ≈ $103,500** at WashU.
- Combined with 3 years at Hobart ($41K/yr scholarship, ~$82K tuition / ~$138K all-in), the WashU 3:3 path delivers BA + BS + MS for **~$186K total tuition / ~$302K all-in (with R&B) over 6 years** — the best cost-per-degree on the board, and roughly the same total all-in cost as Columbia 3:2 despite adding a master's degree.

#### WashU rowing

- WashU Rowing is a **student-run club** program (not NCAA varsity), but **nationally competitive within the club division.**
- Competes in **ACRA (American Collegiate Rowing Association)** — the de facto national championship for club programs. 2025 field: **77 clubs, 388 entries.** Top of the men's side includes UVA (2025 M8 champion), UCLA, Purdue, Rutgers, Michigan, Bucknell — programs that rival or exceed many DIII varsity programs in training volume and racing quality.
- **2024 ACRA Championship results (men's side):**
  - **Men's 4+ : 1st in Grand Final — ACRA national champion**
  - Men's 8+: 14th overall (2nd in C Final) — mid-pack in the flagship eight
- **2025 ACRA Championship results:**
  - **Men's 2- : 3rd in Grand Final (bronze medal)**
  - Women's V8+: 4th in Grand Final
  - Women's 2-: 6th in Grand Final
  - Men's Novice 4+: B Final
  - Men's 2x: E Final
- **Pattern:** WashU men's small boats (4+, 2-) race at a genuinely national level. The flagship M8 is mid-pack against a deep ACRA field.
- Competes at **Head of the Charles, Head of the Rock, Head of the Hooch** — legitimate national-level racing calendar.
- **Caveat 1 (category):** No lightweight category at ACRA; openweight club racing only. Jack would be stepping down from IRA-system Hobart racing to national-level club racing. Not a lightweight destination, but not a rowing dead end either.
- **Caveat 2 (eligibility — critical):** ACRA enforces its own 4-seasons-in-5-years clock counting prior intercollegiate competition (ACRA 2025 Regatta Packet, Eligibility Rule #2). Jack's 3 Hobart IRA seasons burn 3 of 4 ACRA seasons. He has **1 ACRA season of eligibility remaining when he arrives at WashU** — Y4 only. Y5–Y6 at WashU as a grad student are eligibility-exhausted. See the dedicated eligibility section below for the full rule analysis.

#### Open item on WashU

- Whether Hobart–WashU is structured strictly as 3-3 (BS+MS) or whether a 3-2 BS-only option exists. General WashU Dual Degree is 3-3; confirm with Hobart liaison.

---

### 3e. RPI (via 3:2)

> **⚠ Status (updated 2026-04-24): CLOSED — no HWS partnership.** Spector's 2026-04-24 email confirmed no joint-degree relationship exists with Rensselaer. HWS's 2024–25 catalogue Joint Degree Programs page lists only Columbia and Dartmouth; RPI's known 3+2 partner list does not surface HWS. Deep research below pre-dates both findings and is kept as the historical research base only. Closed-path archive lives in [`hobart-unconfirmed-paths.md`](./hobart-unconfirmed-paths.md); verbatim Spector quote in [`hobart-questions.md` §1](./hobart-questions.md#1-prof-ileana-dumitriu--hws-engineering-liaison).

- **Role in Jack's plan (if confirmed):** Fallback partner for 3:2. High-probability admit for prepared students.
- **Degree outcome:** BA from Hobart + BS in engineering from RPI.

#### RPI 3:2 admission

- Required GPA ~3.00.
- Not competitive in the Ivy sense; **high success rate for prepared students.**
- Effectively a floor-level safety option for the 3:2 portfolio.

#### RPI rowing

- **Rensselaer Crew Club** — student-run club with explicit **"Varsity"** and **"Recreational"** tracks listed side-by-side on their website.
- Practice: **three mornings/week (Tue/Thu/Fri, 6:00–7:30 AM) in the erg room.**
- **Spring 2026 racing schedule is listed as "TBD"** on the team site as of this research.
- Known 2024 race entries limited to local regattas (e.g., Head of the Hudson with women's collegiate 4+, men's open/masters 2x, men's collegiate 8+).
- **No evidence of ACRA or IRA participation** in recent years.
- **Assessment: recreational-level club, not a competitive rowing environment.** If Jack transfers to RPI, assume rowing effectively ends.

#### RPI cost

- Full sticker at RPI for years 4–5 (~$67K/yr).

---

### 3f. WPI (Worcester Polytechnic Institute)

- **Location:** Worcester, MA
- **Role in Jack's plan:** 4-year BS Engineering directly; or starting point for attempted Ivy transfer.

#### WPI academics

- ABET-accredited engineering from day 1.
- **US News rank:** #84 National Universities.
- Flagship program: Mechanical Engineering.
- Project-based curriculum, known for hands-on learning.
- Aerospace engineering available (smaller program).

#### WPI rowing

- NCAA DIII program. **Important:** NEWMAC does not run a men's rowing championship — only women's. WPI men are technically a NEWMAC member but compete on the **New England DIII invitational circuit** and target the **IRA DIII National Championship** (held on the Cooper River in Camden, NJ) as their peak event.
- Historically strong **women's** program (2022 NCAA DIII national champions, raced at Women's Henley); women won the 2025 NEWMAC championship as silver medalists to Wellesley.
- **Men's program is mid-pack in DIII and a step below Hobart.** At the 2025 NIRC V8 heat: Trinity 5:43, MIT 5:47, Hobart 5:53, Vassar 6:00, WPI 6:02. **Hobart beat WPI by ~9 seconds.**
- **Not selected to 2025 IRA DIII Championships.** IRA DIII 2025 was won by Trinity; Points Trophy top 8 were Tufts, Trinity, Williams, Bates, Wesleyan, Colby, Ithaca, Marietta — WPI did not make the field. At NIRC (the resume-building regatta used to earn IRA DIII bids), WPI's V8 was relegated to a 3rd-level final, where they finished 2nd behind Hamilton.
- No dedicated lightweight racing.

#### WPI cost

- Tuition: ~$63,046/year
- Room & board: ~$18,900/year
- **Merit scholarship offered: $27,000/year** → net tuition ~$36,046/year
- 4-year total tuition (post-scholarship): ~$144K
- 4-year total cost including room/board: ~$220K

#### WPI → Ivy transfer — why this is structurally bad

The 3-2 cooperative programs exist precisely to avoid brutal regular-transfer admit rates. Comparing:

| Path | Effective admit rate | Predictability |
|---|---|---|
| Columbia 3-2 via Hobart | Competitive review | Moderate |
| RPI 3-2 via Hobart | High | High |
| WashU 3-3 via Hobart | ~90% if criteria met | Near-automatic |
| WPI → Cornell Engineering (straight transfer) | ~10% or less | Low |
| WPI → Columbia SEAS (straight transfer) | ~5–6% | Very low |
| Cornell Fall 2023 overall transfer | 12.6% (834/6,581) — engineering runs lower | Low |
| Columbia Fall 2023 overall transfer | 5.75% (187/3,254) — SEAS competitive within | Very low |

Cornell Engineering is **gated on specific prereqs** — missing any one (Calc I/II, calc-based Physics I with lab, Engineering Gen Chem with lab) disqualifies the application. Cornell allows admission at end of year 1 (sophomore standing) or end of year 2 (junior standing), March 15 deadline.

**Strategic heuristic:** The mixed path — WPI with Ivy transfer as the plan — combines the weaknesses of both and achieves the worst odds of each. If Ivy is the target, work the 3-2 pipeline via Hobart. If engineering-from-day-1 is the target, go to WPI and commit.

---

### 3g. UBC (University of British Columbia)

- **Location:** Vancouver, BC, Canada
- **Role in Jack's plan:** 4-year BASc Engineering as an international student.

#### UBC academics

- **QS World Ranking 2026:** #40 overall.
- **Engineering:** #25 globally, #2 in Canada.
- CEAB-accredited (Canadian equivalent to ABET; widely recognized).
- Co-op program available.
- **Lower brand recognition for US employers** compared to Columbia or Cornell. Strong in Canada, Asia.

#### UBC rowing

- **Canada's most successful varsity rowing program.**
- Men's team won **7 of last 9** Canadian University Rowing Championships.
- **40+ Olympic medals historically.**
- Head of the Charles: 24th in 2024 Championship Eights (against Cambridge, Olympic medalists, etc.).
- Won **Lightweight 4+ at HOCR in 2022.**
- Canadian university rowing has **no dedicated lightweight category** in regular-season racing. Lightweight-specific entries exist only at select head races (e.g., HOCR LW 4+).
- Head coach: Mike Pearce.

#### UBC rowing — what it means for Jack specifically

- **Primary concern: walk-on risk.** UBC rowing is an elite recruited-varsity program, and the walk-on process is competitive. **Jack's own assessment is that he's at real risk of not making the team at all** — not just at risk of not landing in a top boat.
- **Secondary concern: lightweight ceiling.** UBC carries only **4 lightweight roster slots**. Even if he walked on, landing in a LW boat would be unlikely — and without a LW slot he'd be training openweight at ~155 lbs against 185+ lb rowers.
- **Realistic base case: no UBC rowing.** The walk-on risk is the dominant factor; the 4-slot LW ceiling compounds it.
- **Implication for the decision:** UBC should be treated as an **engineering-first** option, with rowing as an upside if he walks on, not as a core-fit rowing path.

#### UBC rowing eligibility for US students

- No nationality restrictions.
- U Sports allows **5 years of eligibility** (more generous than IRA's 4).
- Must be enrolled in 3 courses / 9 credit hours during competing term.
- **Athletic Financial Awards available**, capped at tuition + fees — unlike NCAA DIII, UBC can offer athletic scholarship (*if* he makes the team — irrelevant to walk-on risk).
- No transfer issues since Jack would enter as a freshman.

#### UBC cost

- International engineering tuition: CAD $66,200/year.
- Housing, food, fees, insurance in Vancouver: ~CAD $18–20K/year.
- Total: ~CAD $85K/year.
- **At ~$0.73 CAD/USD exchange rate: ~$62,000 USD/year total; tuition alone ~$48,300 USD/year.**
- **4-year total tuition: ~$193K USD; 4-year total all-in (tuition + on-campus residence + meal plan): ~$268K USD.**
- UBC caps international tuition increases at 3%/year for up to 4 years (predictable).

---

### 3h. SDSU (San Diego State University) — last-resort state school for BSME

- **Role in Jack's plan:** Post-Hobart transfer for ABET BSME. Relevant for Option 2f (Hobart BA → State School BSME).

#### SDSU ME — admission requirements (for major change/transfer)

To transfer into Mechanical Engineering major at SDSU (from another major, including Physics BS):

1. Complete core courses at SDSU with minimum **C grade**:
   - MATH 150 (Calculus I, 4 units)
   - MATH 151 (Calculus II, 4 units)
   - PHYS 195 (Principles of Physics I, 3 units)
   - PHYS 196 (Principles of Physics II, 3 units)
   - CHEM 202 or CHEM 200 (General Chemistry, 4 units)
   - ME 200 or AE 200 (**Statics**, 3 units)
2. Minimum **15 units completed at SDSU** that count toward the BSME degree.
3. Minimum **2.7 overall GPA**.
4. No more than two course retakes on degree audit.

#### SDSU cost

- **Jack is a California resident.**
- CSU systemwide resident tuition (2025–26): $6,450/year.
- SDSU mandatory campus fees add ~$2.3K/year → **~$8.7K–$9K/year all-in**.
- **Two-year stay post-Hobart: ~$18K total.** Dramatically cheaper than any out-of-state path.
- No residency establishment risk — Jack already qualifies for in-state tuition day 1.

#### SDSU rowing

- Club crew only. Not relevant for elite rowing ambitions.

#### Other Cal State BSME options to consider

Any ABET-accredited CSU BSME program can substitute for SDSU at similar in-state pricing (~$9K/yr). Candidates worth a look if SDSU's transfer window or prereq path is awkward:

- **Cal Poly SLO** — BSME is highly impacted; transfer admission is very competitive, but the program is the strongest engineering brand in the CSU system.
- **San José State (SJSU)** — well-regarded BSME, Silicon Valley location, reasonable transfer paths.
- **CSU Long Beach, CSU Fullerton, Sacramento State** — all offer ABET BSME at CSU in-state pricing.

Transfer prereq requirements vary by campus. SDSU's requirements (Calculus I–III, linear algebra, ODEs, PHYS 195/196, Chem, and **Statics**) are a reasonable benchmark.

---

### 3i. WSU (Washington State University) — not cost-competitive for CA residents

Previously considered as an alternative BSME transfer destination. **Dropped from the primary cost model** because Jack is a California resident and WSU non-resident tuition (~$30K/yr) is dominated by any Cal State at in-state pricing (~$9K/yr). WSU stays on the list only as a geographic-preference wildcard (Pullman/Pacific Northwest) if that preference is strong enough to pay a ~$40K premium over SDSU for two years.

---

### 3j. Syracuse University (via 4+2 pathway) — confirmed partnership, MS-only

- **Location:** Syracuse, NY (~1 hr from Hobart)
- **Role in Jack's plan:** **Third primary dual-degree / direct-admission path** (alongside Columbia 2a and Dartmouth 2b) after WashU 3:3 (2c) and RPI 3:2 (2d) were demoted to unconfirmed-partnership status (see [`hobart-unconfirmed-paths.md`](./hobart-unconfirmed-paths.md)). Syracuse is the **only confirmed graduate-engineering pathway** with a published HWS partnership.

#### Syracuse 4+2 partnership — structure

In a March 2026 announcement, Syracuse's College of Engineering and Computer Science (ECS) launched 4+2 master's pathway partnerships with six NY partner schools (HWS, Canisius, Marist, Mercy, Nazareth, Utica). Structure: complete a BA/BS in **science or mathematics** at Hobart (4 years), then enter Syracuse ECS for an MS in engineering (typically 24 months / ~30 credits).

- **Admission gate:** 3.2 cumulative Hobart GPA + faculty recommendation + science or math major → **direct admission** (similar to WashU's "near-automatic if criteria met" standard).
- **Bridge course:** required for students from non-engineering BAs (physics BA qualifies; bridge may be waived for certain well-prepared students).
- **12 MS programs available:** biomedical engineering, chemical engineering, mechanical engineering, civil engineering, environmental engineering, computer engineering, computer science, cybersecurity, among others.

#### Syracuse 4+2 — what it actually delivers

Unlike the existing 3:2 (Columbia, RPI) or 3:3 (WashU) partnerships, the Syracuse agreement skips the ABET BS entirely. **Jack would finish with a Hobart BA in physics + Syracuse MS in engineering — no BS.** This is structurally similar to the Hobart BA + post-service MS path (Option 2g), but without the military service.

- **Engineering reputation:** Syracuse ECS is ranked ~#70 for engineering (US News) — below WashU (~#42), RPI (~#35), Columbia (~#21), Dartmouth (~#51), and roughly tied with WPI (~#64).
- **ABET status concern:** an MS-only engineering credential without an ABET-accredited BS is a weaker profile for early-career engineering roles and is **disqualifying for Professional Engineer (PE) licensure in most states**, which requires the BS to be ABET-accredited.

#### Syracuse rowing — structurally doesn't extend rowing

- Syracuse men's rowing is a **heavyweight-only IRA program** (EARC conference; V8 finished 6th at 2025 IRA Nationals; #7 2026 preseason IRCA/IRA poll). **No lightweight program.**
- By the time Jack arrives at Syracuse (Y5), he has already used 4 IRA seasons at Hobart. **Under the IRA/NCAA 4-in-5 clock, his eligibility is exhausted** — same constraint that binds the Ivy and ACRA paths.
- **Result: 0 rowing seasons at Syracuse.** Same outcome as Option 2f (Cal State) or 2g (OCS) — rowing ends with the Hobart BA.

#### Syracuse cost

- **Tuition (on-campus, 2025–26):** $2,015/credit × 18 credits/yr ≈ **~$36,270/yr tuition**.
- **Housing and food (Syracuse Graduate COA):** ~$16,338/yr.
- **All-in per year (tuition + R&B):** ~$52,600/yr.
- **2-year MS all-in:** ~$105K.
- **Combined 6-year total (Option 2e):** Hobart 4 yrs at ~$46K (post-scholarship) = ~$184K + Syracuse 2 yrs at ~$52.6K = ~$105K → **~$289K all-in.**
- **No published partner-school scholarship or tuition discount.** The Syracuse announcement emphasizes "streamlined pathway" but makes no commitment on financial aid. This is an open item to confirm — even a 25% discount would improve the cost competitiveness materially.

#### Strengths and weaknesses of Syracuse 4+2 vs. the Ivy paths

- **vs. Columbia 3:2 / Dartmouth 2:1:1:1 (the two Ivy paths):** materially lower admit risk (3.2 GPA vs. 3.50 / ~3.30 with competitive review). 2e arrives at 6 years instead of 5. Credential profile is weaker (Syracuse #70 + MS-only vs. Ivy ABET BS/BE), and rowing ends at Hobart Y4 (no Ivy LW year).
- **vs. Cal State BSME (Option 2f):** 2f delivers an ABET-accredited BSME, which is the baseline credential for engineering employment and PE licensure. Syracuse's MS-without-BS is structurally worse for early-career engineering roles. Tradeoff: 2e is a stronger graduate brand (Syracuse vs. CSU), 2f is the stronger employability credential.
- **vs. Option 2g (OCS → post-service MS):** structurally similar end-state (BA + MS) without military service, but at higher out-of-pocket cost (~$289K vs. ~$184K) and weaker grad-program ranking (Syracuse #70 vs. Dartmouth Thayer / Cornell / Stanford via GI Bill + Yellow Ribbon).

**Critical gap to confirm:** the Syracuse pathway accepts BAs in *science or mathematics*. Hobart's physics BA qualifies, but a bridge course is required for non-engineering BAs — confirm whether the bridge is waived for a strong physics-major applicant or if it adds a term to the MS. Also confirm whether any partner-school tuition discount exists (none currently published; even a 25% discount would shift the cost calculus meaningfully).

**Decision-grade analysis:** for the partnership-vs-open-market-MEng comparison (when 2e is the right primary target vs. when it's dominated by Cornell MEng / Dartmouth Thayer MEng / BU / Northeastern at similar cost), see [`hobart-syracuse.md`](./hobart-syracuse.md).

---

## 4. Rowing analysis — detailed

### Jack's rowing priority

Jack wants to be in a **competitive rowing program with a real rowing culture** — B/C boat at a serious program is fine; top boat at a recreational club is not. Being a lightweight is still part of his identity, but **dedicated lightweight racing is not a requirement** — programs that race openweight can still fit his ambition *if* the program structure actually makes a competitive boat available to a lightweight.

**Important caveat for lightweights:** "B/C boat at a serious program is fine" only works when the program has enough LW depth that Jack can reasonably land in a race-participating boat at his weight class, **and** when he's likely to make the team in the first place. That's true at the Ivy LW programs (Columbia, Dartmouth) and to a lesser degree at nationally competitive ACRA clubs (WashU). **It is not true at UBC**, where the more fundamental concern is that **Jack himself is not confident he can walk on** — UBC rowing is a recruited-varsity program with a competitive walk-on process. Even if he walked on, the program carries only 4 LW roster slots, further constraining outcomes.

Lightweight-category context for reference: dedicated lightweight racing exists only in the **US IRA system**. Not in Canadian university rowing. Not in ACRA club regular-season racing. Not at DIII programs. It's a strong plus at the top of the stack (Options 2a and 2b, the Ivy paths) and not the sole filter, but a program's LW roster depth is a hard filter.

### Competition-level comparison

| Program | Level | Lightweight category? | Meets Jack's "competitive culture" bar? | Notable results |
|---|---|---|---|---|
| Columbia | Elite Ivy LW | **Yes, dedicated** | **Yes, elite** | #1 nationally 2023; LW V8 national champion 2022 |
| Dartmouth | Elite Ivy LW | **Yes, dedicated** | **Yes, elite** | 2025 IRA silver medalist (best in program history); V4 Eastern Sprints silver 2025 |
| UBC | Elite Canadian varsity | No — openweight (4 LW roster slots) | **No for Jack** — walk-on is uncertain (Jack's own assessment); even if he walks on, only 4 LW slots | 7 of last 9 Canadian national titles; Olympic pipeline; HOCR Championship Eights competitor |
| Hobart | IRA system D1/D2 level | No — openweight only | **Yes, strong** | #24 2025 coaches poll; moving to Dad Vail 2026 |
| WashU | ACRA club (nationally competitive) | No — openweight only | **Yes, strong (nationally)** | 2024 ACRA **Men's 4+ national champion**; 2025 ACRA **Men's 2- bronze** (Grand Final). M8 is mid-pack in the 77-club ACRA field. Top ACRA programs (UVA, UCLA, Purdue, Rutgers, Michigan) rival or exceed many DIII varsity programs. |
| Navy (USNA) | Patriot League DI | Lightweight exists, smaller | **Yes** | Navy LW competes at Eastern Sprints |
| WPI | DIII IRA system (NEWMAC is women's-only for rowing) | No — openweight only | **Partial** (mid-pack DIII, tier below Hobart and WashU M-small-boats) | **Not selected to 2025 IRA DIII Championships.** #9 D3 coaches poll 2025; beaten by Hobart by 9s at NIRC V8; V8 relegated to 3rd-level final at NIRC. |
| RPI | Recreational club | N/A | **No** | No ACRA/IRA entries; 2024 raced Head of the Hudson only; Spring 2026 race schedule TBD |
| SDSU | Club | N/A | **No** | Not competitive |
| WSU | Club | N/A | **No** | Not competitive |

### Key events explained

- **NIRC (National Invitational Rowing Championship):** Held annually at Lake Quinsigamond, Worcester, MA. Regular-season championship for smaller programs and qualifier for IRA DIII Championships.
- **Dad Vail Regatta:** Largest collegiate regatta in US, held on Cooper River, Pennsauken NJ. As of the 2026 season, offers 4 automatic IRA qualifier spots. Hobart is moving here.
- **IRA National Championship:** De facto national championship of US men's rowing since 1895. Includes dedicated lightweight events (V8, 2V8, 4+). Entry via conference championships and at-large bids.
- **Eastern Sprints:** Ivy League rowing championship. Elite competition level.

### IRA and Ivy League rowing eligibility (critical for all 3:2 and 2:1:1:1 paths)

The governing bodies here are **IRA (Intercollegiate Rowing Association)** for competition rules, and the **Ivy League** for conference-specific overlays at Columbia and Dartmouth. **The NCAA is not directly the governing body** for men's heavyweight or lightweight rowing at Columbia, Dartmouth, or Hobart — the NCAA manual is the IRA's adopted baseline, but all eligibility questions are decided under IRA bylaws and (for Ivy schools) conference policy.

#### IRA eligibility rules (apply at all member schools including Hobart, Columbia, Dartmouth)

- IRA Eligibility & Operating Bylaws (irarowing.com) state: "no specific mention of an amendment to an NCAA rule in the IRA's Eligibility & Operating Bylaws should always be interpreted as the IRA following the rule as it is written in the NCAA manual." So IRA adopts the NCAA framework by default.
- **4 total seasons of competition** across a 5-calendar-year clock from first full-time enrollment.
- Freshman-level competition counts as a season used, even though freshmen can only race in freshman/novice events or V4/Open-4 (not Varsity 8 or 2V8).
- IRA itself does not have a separate redshirt policy different from the NCAA baseline. In principle, a redshirt year is possible at a non-Ivy IRA school.
- IRA does not bar graduate-student competition. Grad students do appear on IRA rosters and IRA All-Academic lists at non-Ivy schools; Columbia's 2022 IRA All-Academic list included a GS-designation athlete on the heavyweight side during the 2021–22 COVID-waiver year.
- IRA governs competition: Eastern Sprints (EARC), Dad Vail, IRA National Championship. Hobart competes in IRA despite being an NCAA Division III school overall.

#### Ivy League conference overlay (Columbia, Dartmouth — NOT Hobart)

This is the layer that actually ends Jack's Ivy rowing at Y5. Ivy League policy is stricter than IRA and trumps it at Ivy schools:

1. **Graduate students are ineligible for varsity athletics.** Ivy League conference policy restricts athletic opportunities to undergraduates. The one-time COVID waiver for the 2021–22 season let that year's seniors race as grad students and was explicitly not renewed ("this change is a direct result of the pandemic and will not be available in future years"). Columbia's 2025–26 men's lightweight roster has no GS designations — confirmation the rule is currently enforced.
2. **No redshirts allowed.** If an athlete is enrolled and medically able during the season, that season counts as used regardless of whether he competes. This is an Ivy overlay, not an IRA rule — IRA/NCAA would otherwise permit redshirting.
3. **Four-year expectation.** Student-athletes are expected to use their 4 seasons within their first 4 undergraduate years. This is stricter than the IRA/NCAA 5-year clock.
4. Limited medical-hardship and leave-of-absence exceptions exist but require Ivy compliance office approval — not a general strategy.

Sources: IRA Eligibility & Operating Bylaws (irarowing.com, 2020 rev.); Cornell Big Red Athletics (Athletic Eligibility and Compliance, 2024); Yale Bulldogs Student-Athlete Handbook Compliance (2024); Daily Pennsylvanian and Harvard Crimson coverage of the 2021 COVID waiver and its subsequent non-renewal; Columbia 2025–26 men's LW roster (gocolumbialions.com) for current-state confirmation.

#### Why the distinction between IRA and Ivy matters for Jack

- If the constraint were IRA-only (i.e. Jack went from Hobart to a non-Ivy IRA school), **a Y4 redshirt + Y5 race plan would work** — he'd have 1 season of IRA eligibility remaining and the 5-year clock accommodates it.
- The reason the redshirt plan doesn't work is **not IRA**, it's the **Ivy League conference overlay**. Columbia and Dartmouth apply the undergrad-only and no-redshirt rules independently of IRA. So the eligibility ceiling at the Ivy destination is lower than what IRA alone would allow.
- A non-Ivy IRA landing spot (e.g., MIT, BU, Georgetown, Gonzaga, Mercyhurst, UPenn for heavyweight) would behave differently — Penn is Ivy so same constraint there; MIT/BU etc. follow NCAA-baseline IRA rules. Not relevant to this decision since the Ivy destinations are Columbia and Dartmouth, but worth flagging if the search ever opens up.

#### Consequence: what each path actually delivers for rowing

**Option 2a: Hobart → Columbia (3:2)**

| Year | School | Rowing | IRA seasons used | Rule determining outcome |
|---|---|---|---|---|
| 1 | Hobart | Freshman season | 1 of 4 | IRA freshman eligibility |
| 2 | Hobart | Varsity | 2 of 4 | IRA baseline |
| 3 | Hobart | Varsity | 3 of 4 | IRA baseline |
| 4 | Columbia | **Columbia LW varsity (undergrad senior)** | 4 of 4 | IRA + Ivy undergrad status |
| 5 | Columbia | **Ineligible — graduate student** | — | **Ivy League conference policy** |

- **Prior plan of "redshirt Y4, race Y5" does not work.** The Ivy League does not allow redshirts, and the Ivy conference policy bars grad-student varsity competition at Y5. (IRA alone would permit the redshirt plan — it's the Ivy overlay that blocks it.)
- **Result: 1 year of Columbia LW racing (Y4). 4 total seasons of competitive rowing (3 Hobart IRA + 1 Columbia LW).**

**Option 2b: Hobart → Dartmouth (2:1:1:1)**

| Year | School | Rowing | IRA seasons used | Rule determining outcome |
|---|---|---|---|---|
| 1 | Hobart | Freshman season | 1 of 4 | IRA freshman eligibility |
| 2 | Hobart | Varsity | 2 of 4 | IRA baseline |
| 3 | Dartmouth | **Dartmouth LW varsity (undergrad junior)** | 3 of 4 | IRA + Ivy undergrad status |
| 4 | Hobart | Openweight (optional) | 4 of 4 if he rows | IRA baseline, openweight only |
| 5 | Dartmouth | **Ineligible — graduate student** | — | **Ivy League conference policy** |

- **Y4 dilemma:** Jack can row openweight at Hobart for a 4th season, but this burns his final IRA eligibility year, and it's openweight (no LW category at Hobart). Alternatively he sits Y4 out and finishes Y5 without rowing either way.
- **Result: 1 year of Dartmouth LW racing (Y3). 3–4 total seasons of competitive rowing depending on Y4 decision.**

**Option 2c: Hobart → WashU (3:3)**

ACRA is not governed by IRA or NCAA, but it has its own written eligibility rules that are more restrictive than sometimes assumed. From the **ACRA 2025 Regatta Packet, Athlete Eligibility section:**

- **Rule #1:** "All participants must be full-time **undergraduate students**... making normal progress toward his/her **first degree.**"
  - **Exception:** "A student-athlete who has received a Baccalaureate or equivalent degree, and who is enrolled in graduate or professional school... **may participate in ACRA, provided the student-athlete has athletic eligibility remaining, and such participation occurs within the time period set forth in number 2 below.**"
- **Rule #2:** "An ACRA competitor has **four years of eligibility, which must be completed during the first 10 semesters or 15 quarters** after the student is enrolled in a collegiate institution... **Competing for an institution in any one event within the academic year is enough to constitute one year of eligibility.**"
- **Rule #5:** "A student-athlete who is enrolled in a graduate or professional school of the college or university may participate, provided he/she has eligibility remaining and is **within five calendar years of initial full-time collegiate enrollment.**"

**Applied to Jack's Hobart → WashU path:**

| Year | School | Rowing | ACRA seasons used | ACRA clock year |
|---|---|---|---|---|
| 1 | Hobart (IRA) | Freshman season | 1 of 4 | 1 of 5 |
| 2 | Hobart (IRA) | Varsity | 2 of 4 | 2 of 5 |
| 3 | Hobart (IRA) | Varsity | 3 of 4 | 3 of 5 |
| 4 | WashU (ACRA) | **WashU ACRA — senior undergrad, uses 4th season** | 4 of 4 | 4 of 5 |
| 5 | WashU (grad) | **Ineligible — 0 seasons remaining** | — | 5 of 5 |
| 6 | WashU (grad) | **Ineligible — outside 5-year clock** | — | out |

- Prior IRA competition at Hobart counts against the ACRA clock under Rule #2's plain language ("competing for an institution" = one season used).
- The grad-student exception (Rules #1/#5) requires *both* eligibility remaining *and* within the 5-year window. Jack has neither by Y5.
- **Result: 3 seasons at Hobart + 1 season at WashU (Y4) = 4 total seasons of competitive rowing.** This is **the same total as the Ivy paths (2a, 2b), not 6.**

**This corrects the earlier model in this doc that assumed 3 additional ACRA years at WashU.** ACRA's own written bylaws prevent that. What distinguishes the WashU path from the Ivy paths is not *more years of rowing* but: cheaper cost, lower admit risk, adds an MS degree, and Y4 is at a nationally competitive ACRA club (2025 Men's 2- bronze) rather than an elite Ivy LW program.

**Open question for the WashU coach conversation:** whether ACRA enforces Rule #2 strictly against transfers with prior IRA competition, or whether there is a school-by-school practice where coaches petition the ACRA eligibility committee (Bob Jaugstetter, Kyle Dowd) for prior-competition exceptions. The written rule is explicit; whether any case-by-case leniency exists is worth asking. If the rule is enforced as written, the WashU path gives 1 year of WashU racing, not 3.

**Option 1: WPI**

- NCAA DIII program. NEWMAC does not run a men's rowing championship, so WPI men compete on the New England DIII invitational circuit and target the **IRA DIII National Championship** as their peak event (when selected — WPI was not selected in 2025). 4 seasons in 5 years under NCAA rules. With a 4-year BS, he uses all 4 naturally. No eligibility constraints.

**Option 3: UBC (U Sports, not NCAA/IRA)**

- U Sports allows **5 years of eligibility** — more generous than NCAA/IRA.
- Not a constraint if he makes the team. But see primary concern: walk-on risk and 4 LW roster slots.

#### The key corrections to the prior plan

**Correction 1: Ivy paths deliver 1 year of Ivy LW racing, not 2.**

The old plan assumed Jack could redshirt Y4 at Columbia and race both Y4 and Y5 as part of the 3:2 experience. **This is not possible** — not because of IRA (IRA alone would allow it), but because of **Ivy League conference policy**: no redshirts, and Y5 is graduate standing (ineligible for Ivy varsity). The same constraint applies to Dartmouth's Y5. Both Ivy paths deliver **one year of elite Ivy LW racing**, not two.

**Correction 2: WashU 3:3 delivers 1 year of ACRA racing, not 3.**

The old plan assumed ACRA club rowing had informal or enrollment-based eligibility and that Jack could row all 3 grad years at WashU. **The ACRA 2025 Regatta Packet eligibility rules are explicit:** 4 seasons in 5 calendar years from initial full-time enrollment, with "competing for an institution in any one event within the academic year" constituting a used season. Prior IRA competition at Hobart counts. Jack burns 3 of 4 ACRA seasons at Hobart, uses the 4th at WashU in Y4, and is eligibility-exhausted by Y5.

**Combined result: the two active rowing-fit options (2a Columbia, 2b Dartmouth) each deliver 4 total competitive seasons.** What differs is peak level of the final season (Columbia LW vs. Dartmouth LW), credential earned (Columbia SEAS BS vs. Dartmouth Thayer BE), all-in cost (~$304K Columbia / ~$316K Dartmouth — both Dartmouth years are billed at full Dartmouth tuition; the prior model that routed Y3 through Hobart at the scholarship rate has been rejected, see [Option 2b](#option-2b-hobart--dartmouth-2111-5-years) below), and admit risk (3.50 vs. ~3.30 gate, both competitive transfer reviews). Neither is "sustained elite rowing across 5–6 years" — the 4-in-5 eligibility clock binds in both cases. The previously-tracked WashU 3:3 (2c) third rowing-fit option has been **closed** per Spector's 2026-04-24 email (no HWS partnership) — see [`hobart-unconfirmed-paths.md`](./hobart-unconfirmed-paths.md). Even if it had existed, ACRA's 4-in-5 clock would have made it a 4-total-season path with only 1 ACRA year at WashU, not a true rowing expansion.

This does not invalidate any of these paths. Each still provides a real, multi-year competitive rowing experience and a strong engineering credential. But it means the **choice between Ivy paths and WashU 3:3 is not "elite 1-year experience vs. 3 extra years of rowing."** It's a straightforward tradeoff: Ivy credential + higher admit risk + higher cost vs. WashU BS+MS + near-automatic admit + lower cost. Rowing is not a meaningful tiebreaker between them at the total-seasons level.

### UBC eligibility (U Sports)

- No nationality restrictions.
- 5 years of eligibility (more generous than IRA).
- Must be enrolled in 3 courses / 9 credit hours during competing term.
- Athletic Financial Awards available, capped at tuition + fees.

---

## 5. Cost analysis — detailed (all in current USD, tuition + on-campus room & board)

The cost model is **tuition + on-campus room & board**, rounded to the nearest ~$K in summary tables. Books, personal expenses, travel, loan origination, and required campus fees are excluded — they're roughly path-invariant (each adds ~$2–4K/yr regardless of school) and including them adds noise without changing the relative ranking. Jack's parent set the simplification scope.

### Per-school baseline (2025–26 rates)

Assumptions used throughout (merit scholarships are **tuition-only** — they don't reduce R&B):

| School | Tuition (net) | R&B (on-campus) | All-in / yr | Notes |
|---|---|---|---|---|
| Hobart | ~$27,414 ($41,000/yr merit on $68,414 ≈ 60% off) | ~$18,210 (standard room + Finger Lakes meal plan) | **~$46K** | **Scholarship renewal terms need offer-letter confirmation** (GPA minimum, semester cap, and leave-of-absence treatment for Option 2b's Dartmouth Y3). Hobart standard room $9,642 + Finger Lakes meal $8,568. Cheaper meal plan options bring R&B down by $5–7K. |
| WPI | ~$36,046 ($27K merit on $63,046) | ~$18,900 (housing $10,200 + food $8,700) | **~$55K** | Per WPI's published 2025–26 first-year COA. |
| Columbia | ~$70,170 (full sticker) | ~$13,222 (upperclass on-campus + meal plan) | **~$83K** | Columbia first-year R&B is $11,900; upperclass $13,222. Used upperclass as the conservative case. No need-based aid assumed. |
| Dartmouth | ~$68,000 (full sticker) | ~$20,670 (housing $12,579 + food $8,091, all 3 quarters) | **~$89K** | Dartmouth quarter system; full-year R&B is $20,670 across 3 terms. No need-based aid assumed. |
| WashU undergrad | full sticker (~$65K) | ~$20,000 | **~$85K** | Not used in the model — Jack enters WashU as a grad student via 3:3. |
| WashU 3:3 grad years | ~$34,595 (50% Graduate Affiliation Scholarship on $69,190) | ~$20,000 (typical grad off-campus housing + food) | **~$54.5K** | GAS is a structural feature of the 3:3 program, not need-based. |
| RPI | ~$65,000 (full sticker) | ~$18,560 (room $9,750 + meal $8,810) | **~$83K** | RPI double room + standard meal plan. |
| UBC (international) | ~$56,000 USD (CAD ~$66,200 @ 0.73) | ~$11,000 USD (residence ~CAD $7K + All Access Dining CAD $7,250 ≈ CAD $14K → USD ~$11K) | **~$67K USD** | International engineering tuition. UBC R&B based on Place Vanier / Orchard Commons / Totem Park residence + mandatory All Access Dining. Off-campus Vancouver runs higher. |
| SDSU / Cal State (CA resident) | ~$9,000 (in-state tuition + campus fees) | ~$20,000 (on-campus residence + required first-year meal plan; mid-tier room) | **~$29K** | Jack is a CA resident. SDSU first-year on-campus typically $20–22K all-in for room+meal. **If Jack commutes from home, R&B drops to ~$0** — cuts ~$30–40K from the 2-year Cal State portion of Option 2f. |
| USNA (military wildcard) | $0 | $0 | **$0** + monthly stipend | Tuition, R&B, medical all covered. Midshipmen receive a monthly stipend (~$1,200/mo, increases by year). 5-year active-duty service commitment post-grad. |

### Option 1: WPI (4 years)

- All-in: $55K/yr × 4 = **~$220K**
- *(Tuition-only would be $36K × 4 = ~$144K.)*

### Option 2a: Hobart → Columbia 3:2 (5 years)

- Hobart (3 years): $46K × 3 = ~$138K
- Columbia (2 years): $83K × 2 = ~$166K
- **Total all-in: ~$304K**

### Option 2b: Hobart → Dartmouth 2:1:1:1 (5 years)

**Key tuition structure (revised 2026-04-29):** Jack pays **full Dartmouth tuition for both Y3 and Y5** at Dartmouth. The earlier model assumed the Dartmouth Thayer Partner School Dual-Degree page's exchange-year language (home-institution tuition during the first Dartmouth year) would route Y3 through Hobart's scholarship-discounted tuition — that assumption is now rejected. The Hobart $41K/yr merit scholarship does **not** transfer to Dartmouth for the Y3 dual-degree year. Y3 credits still flow back to Hobart for BA-completion purposes, but the financial structure is Dartmouth pricing in both Dartmouth years.

For reference, the Dartmouth Thayer page still says:

> First year: "Students applying for the first year of the Dual-Degree Program should consult with their home institution's registrar regarding tuition... During the first year, eligible Dual-Degree students receive financial aid through their home institutions."
>
> BE year: "Dual-Degree students returning for the BE year are considered Dartmouth students and pay tuition and fees to Thayer School of Engineering."

The "consult with home institution's registrar" language remains accurate as a procedural pointer, but the actual tuition Jack pays for Y3 is full Dartmouth — Hobart's merit scholarship is enrollment-conditional and does not extend to the dual-degree year at Dartmouth.

Housing: Y3 is on-campus in Dartmouth undergraduate housing; Y5 is off-campus in Hanover at the student's own arrangement (modeled at the same R&B figure for simplicity).

- Hobart (3 years total — Y1, Y2, Y4): $46K × 3 = ~$138K (Hobart tuition post-scholarship + Hobart R&B)
- Dartmouth Y3 (full Dartmouth tuition + on-campus R&B): ~$68K + ~$21K = ~$89K
- Dartmouth Y5 (full Dartmouth BE student): ~$68K + ~$21K = ~$89K
- **Total all-in: ~$316K**
- **Now more expensive than Columbia 3:2 (~$304K)**, inverting the prior cost ranking. The 2b vs. 2a case no longer rests on cost — the remaining 2b advantages are the earlier Feb 1 Y2 admission deadline (vs. Feb 1 Y3 for Columbia, enabling the portfolio-strategy fallback), the lower GPA gate (~3.30 vs. 3.50), and the Y3 Dartmouth LW racing year vs. Y4 at Columbia.

### Option 2c: Hobart → WashU 3:3 (6 years) — *closed, no HWS partnership*

> **⚠ CLOSED 2026-04-24** — Spector confirmed no HWS partnership exists. See [`hobart-unconfirmed-paths.md`](./hobart-unconfirmed-paths.md). Cost line below is the historical model and is retained for reference only; not a live option.

- Hobart (3 years): $46K × 3 = ~$138K
- WashU grad (3 years, with 50% GAS): $54.5K × 3 = ~$163.5K
- **Total all-in: ~$302K**
- Would deliver **BA + BS + MS** in 6 years. Roughly the same total cost as Columbia 3:2 despite adding an extra year and a master's degree, because WashU's per-year cost is materially lower.

### Option 2d: Hobart → RPI 3:2 (5 years) — *closed, no HWS partnership*

> **⚠ CLOSED 2026-04-24** — Spector confirmed no HWS partnership exists. See [`hobart-unconfirmed-paths.md`](./hobart-unconfirmed-paths.md). Cost line below is the historical model and is retained for reference only; not a live option.

- Hobart (3 years): $46K × 3 = ~$138K
- RPI (2 years): $83K × 2 = ~$166K
- **Total all-in: ~$304K**

### Option 2e: Hobart → Syracuse ECS 4+2 (6 years)

- Hobart BA (4 years): $46K × 4 = ~$184K
- Syracuse MS (2 years, on-campus, no partner-school discount published): ~$52.6K × 2 = ~$105K
- **Total all-in: ~$289K** for a Hobart BA + Syracuse MS. **No ABET BS.**
- *Scholarship sensitivity:* if Syracuse were to offer a 25% partner-school tuition discount (not currently published but worth asking about), the 2-year grad portion drops to ~$87K and total falls to ~$271K.

### Option 2f: Hobart BA → State School BSME (Hobart Applied Physics + Cal State BSME)

**Jack is a California resident**, so Cal State BSME tuition is at in-state rates. Any Cal State with an ABET BSME and the right prereq path (SDSU, Cal Poly SLO if admissions permit, SJSU, CSU Long Beach, etc.) can substitute for SDSU at similar in-state pricing.

**With full Hobart BA, 6 years (on-campus CSU):**
- Hobart (4 years): $46K × 4 = ~$184K
- SDSU on-campus (2 years): $29K × 2 = ~$58K
- **Total all-in: ~$242K**

**With full Hobart BA, 6 years (commuting CSU from home):**
- Hobart (4 years): ~$184K
- SDSU commuting (2 years, tuition only ~$9K): ~$18K
- **Total all-in: ~$202K** — roughly $40K cheaper if Jack lives at home for the Cal State portion.

**Leaving Hobart after year 3 without BA, 5 years (on-campus CSU):**
- Hobart (3 years): ~$138K
- SDSU on-campus (2 years): ~$58K
- **Total all-in: ~$196K** — sacrifices the Hobart BA to save ~$46K.

### Option 2g: Hobart BA → OCS → post-service MS

- Hobart BA (4 years): $46K × 4 = **~$184K out-of-pocket**
- Military service: paid (officer salary).
- Post-service MS at Dartmouth Thayer / Cornell / Stanford via GI Bill + Yellow Ribbon: **$0 tuition** (housing allowance + books stipend on top).
- **Total all-in: ~$184K** for a Hobart BA + elite engineering MS. Service commitment (3–5 yrs) is the real cost, not the dollar number.

### Add-on to Option 1: WPI → Ivy transfer (highly competitive long shot)

- If transfer lands (end of year 1 or year 2): WPI $55K × 1–2 + Ivy $83K–$89K × 2–3 = **~$300K–$370K all-in**
- If transfer fails: stays at WPI → same as Option 1 (~$220K all-in)

### Option 3: UBC (4 years, international)

- All-in (USD): $67K × 4 = **~$268K USD**
- Tuition increases capped at 3%/yr (predictable). FX risk on Canadian dollar.

### Cost summary table (tuition + on-campus R&B, USD)

| Option | All-in cost | Years | Degrees awarded |
|---|---|---|---|
| 2g. Hobart BA → OCS → MS | ~$184K + $0 grad | 4 + service | BA + post-service MS |
| 2f. Hobart BA → Cal State BSME (commute) | ~$202K | 6 | BA Physics + BSME |
| 1. WPI | ~$220K | 4 | BS Engineering |
| 2f. Hobart BA → Cal State BSME (on-campus) | ~$242K | 6 | BA Physics + BSME |
| 3. UBC | ~$268K (USD) | 4 | BASc Engineering |
| 2e. Hobart → Syracuse ECS 4+2 | ~$289K | 6 | BA + Syracuse MS (no ABET BS) |
| 2c. Hobart → WashU 3:3 *(closed — no HWS partnership)* | ~$302K | 6 | BA + BS + MS Engineering |
| 2a. Hobart → Columbia 3:2 | ~$304K | 5 | BA + Columbia BS |
| 2d. Hobart → RPI 3:2 *(closed — no HWS partnership)* | ~$304K | 5 | BA + RPI BS |
| 2b. Hobart → Dartmouth 2:1:1:1 | ~$316K | 5 | BA + Dartmouth BE |
| Add-on. WPI → Ivy (if transfer works) | ~$300K–$370K | 4 | BS Engineering (Ivy) |

### What's excluded from these numbers (and why it doesn't change the ranking)

- **Books and supplies:** ~$1K–$1.5K/yr per school. Roughly equal across paths.
- **Personal expenses:** Schools quote ~$1.5K–$2.5K/yr. Driven more by Jack than by school.
- **Travel:** Cross-country flights to/from Hobart, Vancouver, or St. Louis are higher than to/from a Cal State, but rounding to per-year doesn't materially change a 5–6 year total.
- **Required fees** beyond the campus fees already bundled into Cal State tuition. Most schools quote $1K–$3K/yr in fees on top of tuition; folding them in would shift each path by $5–15K total but not reorder anything.
- **FX risk on UBC:** modeled at $0.73 USD/CAD. A 10% CAD swing changes UBC's total by ~$25K USD in either direction.

### Military options cost

- **USNA:** Tuition, R&B, medical all covered. Monthly stipend (~$1,200/mo, increases by year). **Net cost: $0** out-of-pocket. Service obligation: 5 years active + 3 years reserve post-commissioning.
- **OCS / NUPOC:** Applied after any civilian degree. OCS itself is a 13-week school at Newport, RI. NUPOC offers signing bonuses of $15K–$38K and pay during school. Service obligation 5–8 years depending on community. Doesn't change the cost of the underlying college path — only what happens after.
- **Post-service MS via GI Bill + Yellow Ribbon (Option 2g mechanism):** $0 tuition at Dartmouth Thayer (uncapped Yellow Ribbon), near-$0 at Cornell, Stanford, and similar. Mechanism detail in `hobart.md` Option 2g.

---

## 6. Military option — the wildcard

Added in response to Jack's parent's note that military is a real consideration (USNA this summer, OCS after Hobart). The military option interacts with every primary path rather than competing with them, but the right entry path depends on **what role Jack actually wants** inside the service. Post-service education benefits (GI Bill + Yellow Ribbon) also make several primary paths dramatically cheaper for the Jack-ends-up-wanting-an-MS case.

### 6.0. The prior question: what does Jack want out of military service?

Officer roles sort roughly into four buckets. The best entry path depends on which of these Jack finds compelling.

#### Bucket 1 — Technical / nuclear

- **Roles:** Submarine Officer (designator 1170), SWO-Nuclear (1160), Naval Reactors Engineer (100B).
- **Day-to-day:** Running a nuclear propulsion plant (sub or carrier) or doing reactor engineering at Naval Reactors HQ / prototype sites. Long sea tours for sub/SWO-N (months underwater for subs); HQ engineering work for NR.
- **Culture:** Small peer group, demanding technical standards, prestige track, "nukes are different" identity inside the Navy.
- **Engineering fit:** Most technical path. Nuclear training itself is effectively a graduate-level engineering course.
- **Best entry:** NUPOC (junior/senior year) or USNA.
- **Degree fit:** Physics BA qualifies. ABET engineering BS also qualifies.
- **Signature compensation:** $5K+/month while still in college, $30K–$38K signing bonus, up to ~$168K before commissioning.
- **Service obligation:** 5 years active + 3 reserve.

#### Bucket 2 — Operational line

- **Roles:** Surface Warfare Officer (SWO, 1160), Naval Aviator (pilot), Naval Flight Officer (NFO), Marine Corps officer (ground or air).
- **Day-to-day:** Leading sailors or Marines, standing watch on a ship, flying aircraft, operational planning, deployments. More "leading people and running operations" than "doing engineering."
- **Culture:** Surface community has lower retention (33% remain after 10 years) and higher separation rates than the nuclear or aviation communities. Aviation and USMC have strong identity cultures.
- **Engineering fit:** Engineering degree helpful but not required. Day-to-day is leadership-and-operations, not technical engineering.
- **Best entry:** USNA or post-grad OCS (ROTC excluded on capacity grounds — see 6c).
- **Degree fit:** Any bachelor's qualifies.
- **Service obligation:** 4 years active (SWO), 8+ years for pilot/NFO due to training pipeline.

#### Bucket 3 — Engineering duty (closest to civilian engineering)

- **Roles:** Engineering Duty Officer (EDO, 1460), Civil Engineer Corps (CEC, 5100), Army Engineer Officer.
- **Day-to-day:**
    - **EDO:** Ship design, ship acquisition programs, maintenance engineering. Moves into program management for naval systems.
    - **CEC:** Public works, base infrastructure, NAVFAC construction projects. Think Bechtel / AECOM work but in uniform. Often co-leads with Seabees.
    - **Army Engineer:** Combat engineering, military construction, infrastructure, USACE civil works projects.
- **Culture:** Technical specialists inside the Navy/Army. Career-long engineering identity rather than rotating through command tracks.
- **Engineering fit:** **Closest to practicing civilian engineering inside the service.** Most direct transfer to civilian engineering careers after.
- **Best entry:**
    - **CEC:** OCS direct accession with ABET BS (or collegiate program with 18 months until graduation).
    - **EDO:** SWO → EDO redesignation (start as SWO, transition after qualifying).
    - **Army Engineer:** OCS, USMA, or **Direct Commissioning** (post-grad, requires ABET BS + 3 years relevant work experience). ROTC excluded on capacity grounds — see 6c.
- **Degree fit:** **Requires ABET-EAC engineering BS** (or architecture NAAB) for engineering-specific designators. **Hobart Applied Physics does NOT qualify for EDO, CEC, or Army Engineer Direct Commission.**
- **Service obligation:** 4 years active + 4 reserve (CEC); 8 years total for Army Direct Commission.

#### Bucket 4 — Special operations / combat arms

- **Roles:** Navy SEAL, EOD, Marine infantry, Army Ranger / Special Forces, combat engineer.
- **Day-to-day:** Physical-first, high-commitment, small-unit combat and specialized operations.
- **Culture:** Selection-based identity; SpecOps communities are tight.
- **Engineering fit:** Engineering not central.
- **Best entry:** USNA, post-grad OCS, or enlisted-to-officer tracks (ROTC excluded on capacity grounds — see 6c).
- **Degree fit:** Any bachelor's qualifies.

#### Key conversation to have with Jack

Which bucket appeals to him? The answer significantly narrows the entry-path decision:

- **Technical / nuclear** → NUPOC is the primary path; USNA works if he also wants the Academy experience.
- **Operational line** → OCS or USNA; engineering degree not required.
- **Engineering duty** → OCS with ABET BS (CEC direct accession); **Option 2f qualifies only after completing the state-school BSME, not after the Hobart physics BA alone.**
- **SpecOps / combat** → any entry path; physical performance dominates.

### 6.1. Can military service pay for a BS or MS in engineering afterward? Yes — three mechanisms.

This is directly relevant to the decision because it meaningfully changes the effective cost math for Option 2f (Hobart BA → State School BSME), Option 1 (WPI), and Option 3 (UBC), and is relevant for anyone who enters the service without the degree they ultimately want.

#### (i) Post-9/11 GI Bill (Chapter 33) — the primary benefit

- **Earned:** 100% benefit after 36 months (3 years) of active duty. Sliding scale from 40% at 90 days to 100% at 36+ months.
- **Tuition coverage (2025–26 rates):**
    - **Public schools (in-state):** 100% of net tuition and mandatory fees.
    - **Private schools:** Capped at **$29,920.95/year** for net tuition and mandatory fees.
    - **Foreign schools:** Capped at same amount.
- **Housing allowance:** Monthly BAH at the E-5 with-dependents rate for the school's ZIP code (can be $2K–$4K+/month in high-cost areas).
- **Books stipend:** $1,000/year.
- **Duration:** 36 months of full-time benefits (effectively a 4-academic-year bachelor's, or 18–24 months of grad school).
- **Transfer to dependents:** Allowed after 6 years of service with an additional 4-year commitment.

#### (ii) Yellow Ribbon Program — private-school top-up

Yellow Ribbon is a dollar-for-dollar match between the school and the VA for tuition above the GI Bill cap. Requires 100% GI Bill eligibility.

| School | Yellow Ribbon contribution | Notes |
|---|---|---|
| **Dartmouth** | **Uncapped, full remaining tuition** | No limit on participants. A+ rating. Covers Thayer School of Engineering fully. |
| **Cornell** | Full remaining tuition | Capped at ~100 veterans on campus; first-come-first-served for grad students. Covers Cornell Engineering. |
| **Columbia** | $8,000/year additional | Most veterans enter via General Studies. Grad (SEAS) Yellow Ribbon participation varies — verify before applying. |
| **Stanford** (reference) | $16K–$20K/year for engineering grad | Uncapped. |
| **WashU** | Offers Yellow Ribbon; varies by school | Graduate engineering participation worth checking. |
| **WPI** | Offers Yellow Ribbon | Covers bulk of tuition for veterans. |
| **Ivies generally** | Most Ivies offer Yellow Ribbon with various caps | Worth checking per-school. |

**Practical implication:** a veteran with 100% GI Bill eligibility can attend **Dartmouth Thayer Engineering, Cornell Engineering, or Stanford Engineering for near-zero tuition** post-service. The Columbia gap is larger ($8K YR + $30K GI Bill vs. $70K tuition ≈ $32K/year uncovered), so Columbia is a weaker fit for the post-service MS path.

#### (iii) Navy-funded graduate education (active duty)

Available *during* service, before separation, for selected officers.

- **Fleet Scholars Education Program (FSEP):** Navy pays full tuition and the officer continues drawing salary for up to 24 months at any accredited U.S. grad school (MIT, Stanford, Purdue, Michigan, etc.) for a master's. Competitive selection. Typically O-3 / O-4 ranks, strong records. Distance learning and executive programs not eligible.
- **CIVINS (Civilian Institutions program):** Umbrella that includes FSEP. 71 subspecialties, 33 fields of study, 100+ participating civilian institutions. ~22% of graduate officer assignments annually go to civilian institutions.
- **Naval Postgraduate School (NPS):** Monterey, CA. Navy-internal grad school, tuition-free, full pay. Offers master's degrees in engineering, operations research, cybersecurity, etc. Standard path for officers seeking a master's while continuing to serve.
- **Tuition Assistance (TA):** $4,500/year cap, $250/semester hour. Useful for part-time coursework, not a full-degree funder.

#### Dollar value of these benefits

Rough estimates of tuition-only value if Jack completes 3+ years of active duty and goes to grad school after:

| Grad program | GI Bill + YR covers | Out-of-pocket tuition |
|---|---|---|
| Dartmouth Thayer (MEng/MS) | $30K + uncapped YR | **~$0** |
| Cornell Engineering (MEng/MS) | $30K + full YR (if slot available) | ~$0 |
| Stanford Engineering MS | $30K + $16K–$20K YR | ~$20K–$25K/year residual |
| Columbia SEAS (MS) | $30K + $8K YR | ~$32K/year residual |
| WashU McKelvey (MS) | $30K + YR (varies) | Varies, likely low |
| State school (in-state MS) | 100% | $0 |

**Total tuition value of the benefit across a BS or MS:** **$120K at a public school; $200K–$300K+ at an elite private engineering program** — before counting the BAH stipend and salary during FSEP/CIVINS.

#### How this changes the cost math for the primary paths

| Primary path | Stand-alone all-in cost | With military service + GI Bill grad school after |
|---|---|---|
| Option 1 (WPI) | ~$220K | Can add Dartmouth/Cornell/Stanford MS for ~$0 post-service. Strong cost efficiency. |
| Option 2g (Hobart BA → OCS / NUPOC, skip the state-school BSME) | ~$184K (Hobart BA only) | Can add **Dartmouth Thayer MS for ~$0** post-service. End state: Physics BA + Navy nuclear service + Dartmouth MEng. |
| Option 3 (UBC) | ~$268K (USD) | Same — post-service MS at Dartmouth/Cornell possible. Upgrades the US-brand-recognition problem of the UBC undergrad. |
| Options 2a/2b (Hobart → Ivy) | ~$304K (2a) / ~$316K (2b) | Less value-add since Ivy BS is already in hand; still available if he wants a further MS. |

**Key insight:** the military path is not just "how Jack pays for his first degree" — it's also a **structured path to an elite engineering MS at near-zero tuition after service.** This is particularly powerful for Options 1 and 2g, where the undergrad credential is less prestigious (or less expensive) than an Ivy BS and can be topped with a Dartmouth/Cornell/Stanford MS for effectively free.

### 6a. USNA (US Naval Academy) — apply this summer

#### Eligibility

- U.S. citizen by July 1 of year of entry.
- At least 17 years old and **not having passed 23rd birthday by July 1 of entry year**.
- Unmarried with no dependents.
- Valid SSN.

#### Application process (same for HS students and college students)

1. Submit preliminary application (open to current college students).
2. **Obtain a nomination** — from a U.S. Senator, Representative, Vice President, or via military-affiliated categories (Presidential, ROTC, enlisted, etc.).
3. Complete the full candidate application.
4. Pass medical examination (DoDMERB).
5. Pass Candidate Fitness Assessment.
6. Complete interview with a Blue and Gold Officer.

#### Timing

- Applications typically open **April** with **January 31 deadline** for the following academic year.
- Jack would be applying in summer/fall 2026 for Class of 2031 (entering July 2027).

#### Critical constraints for an already-enrolled college student

- **USNA does not accept transfer credit.** All admits — regardless of prior college — **start over as plebes and complete the full 4-year program.**
- This means if Jack enrolls at Hobart in fall 2026 and then accepts USNA for fall 2027, his Hobart freshman year yields no academic credit at USNA. He's Class of 2031 instead of 2030.
- Acceptance rate: ~8% of applicants. With nomination hurdle on top.

#### Engineering at USNA

- USNA is an **ABET-accredited engineering school.**
- ~60% of midshipmen major in STEM (engineering, science, math).
- Majors include Aerospace, Mechanical, Electrical, Naval Architecture, Systems Engineering, Ocean Engineering, Nuclear Engineering, and more.
- **Delivers a BS in Engineering with no tuition cost.**

#### Rowing at USNA

- Navy has strong **heavyweight** rowing (Patriot League DI).
- **Lightweight program exists**, smaller roster. Navy LW competes at Eastern Sprints.
- Competitive racing at the DI level.

#### Post-USNA commitment

- Commissioned as an Ensign (O-1) upon graduation.
- **5 years active duty + 3 years reserve** minimum. Longer for Navy pilot/NFO (8 years), submarine nuclear (5 years), etc.

### 6b. OCS / NUPOC after any bachelor's degree

Relevant as an **end-state officer pipeline** regardless of which civilian college Jack attends.

#### Navy OCS — general

- 13-week program at Officer Training Command Newport, RI.
- Age 19–36 at commissioning (waivers to 42).
- U.S. citizen.
- Officer Aptitude Rating (OAR) exam; scores above 45 preferred (waivers to 40 possible).
- **Minimum 2.7 GPA** to be considered.

#### OCS designators and degree requirements

- **Engineering Duty Officer (EDO):** Requires **ABET-EAC accredited engineering degree** (Engineering Accreditation Commission). ABET-ETAC degrees may work with EIT/PE credentials or waivers.
  - **Hobart Applied Physics BA does NOT qualify for EDO.**
  - Columbia/WashU/RPI/WPI/UBC engineering degrees **do qualify** (ABET or CEAB).
- **Unrestricted Line (SWO, aviation, NFO, intel, supply, submarine, etc.):** Requires a **bachelor's in any field.**
  - **Physics is perfectly fine.**
  - Pilot and NFO programs do not require engineering specifically.
- **Nuclear Propulsion Officer Candidate (NUPOC):** Specialized pipeline for nuclear surface, submarine, or Naval Reactors roles.

#### NUPOC — why this matters for the Hobart Applied Physics path

- **Explicitly open to physics majors** with strong technical coursework.
- Eligibility windows:
  - Within 42 months of graduating with a bachelor's degree
  - Within 54 months of completing a combined bachelor's + master's
  - Currently in a master's program within 12 months of earning it
  - Already possess a bachelor's degree
- Degrees must come from **regionally accredited U.S. schools.** (UBC as a Canadian school may not qualify — requires confirmation with recruiter.)
- **Academic standards:** Minimum C in technical coursework; any below-C requires a waiver. Competitive applicants aim for A's and B's.
- **GPA rules of thumb:**
  - Submarine / Surface Warfare applicants: **3.0+**
  - Instructor applicants: **3.4+**
  - Naval Reactors Engineer applicants: **3.7+** (typically for technical majors)
- **Technical coursework expected:** calculus (through DiffEq), calc-based physics, often chemistry. **Hobart Applied Physics covers all of this.**
- **Compensation:** signing bonuses $15K–$38K common; pay during school; officer salary post-commission.
- **Service obligation:** 5–8 years depending on community.
- Must qualify for **secret clearance** (top secret for Submarine and Naval Reactors).

#### Why this changes the calculus on Option 2f

If Jack ends up with only a Hobart Applied Physics BA (because the 3:2 didn't work out and he decided not to spend 2 more years chasing an ABET BSME at a state school), **NUPOC is a legitimate elite officer pipeline** that doesn't require an engineering BS. That turns Option 2f from "failed engineering path" into "physics undergrad → nuclear Navy officer" — making the state-school BSME leg optional rather than required.

### 6c. ROTC — considered and excluded

NROTC / AROTC / AFROTC scholarships can cover full tuition + stipend at participating civilian schools in exchange for a post-grad service commitment. On paper this is attractive for the cost profile of Options 3 and 4.

**However, ROTC is not on the table for Jack because of capacity.** ROTC requires:

- Pre-dawn PT ~3 days/week (0500–0600 typical)
- Weekly military-science class plus evening drill/lab
- Multi-week summer training commitments (ex: NROTC CORTRAMID after sophomore year, cruise blocks after junior year)
- Physical readiness testing and uniform standards year-round

Layered on top of:

- An engineering curriculum (15–18 credits/semester including labs and design sequences)
- A serious rowing program (practice 6x/week, racing season travel, winter erg volume)

**The aggregate load is not compatible with maintaining the GPA needed for 3:2 admission (3.30–3.50) or with performing in a competitive boat.** Something would give — either grades, rowing, or ROTC performance — and any of the three being compromised defeats the point of the decision.

**The practical military entry paths for Jack are therefore:**

- **USNA** (freshman restart, no concurrent civilian-college load)
- **NUPOC** (junior/senior year application; no training load during college beyond application process)
- **Post-grad OCS** (after graduation, no college-period load at all)

ROTC is excluded from the decision matrix and from all cost/path recommendations going forward.

---

## 7. Decision framework — full

### Best case — primary plan works

Hobart + Columbia 3:2 if GPA trends 3.5+. Jack gets the dual-degree outcome he's aiming for with elite lightweight rowing at the end.

### Most likely good outcome — 3:2 portfolio

Apply to **Columbia, RPI, and WashU** junior year. RPI or WashU is effectively a lock if he maintains 3.25+. Dual degree outcome preserved, just with a different engineering school.

### Dartmouth as reach

Apply to Dartmouth 2:1:1:1 sophomore year as a reach. Low downside (cheap to apply, early answer by mid-April). If admitted, commit — but only if confident in year-3 performance given the downstream transcript-damage risk.

### USNA as parallel reach

Apply to USNA summer/fall of freshman year. Accepts ~8% of applicants plus the nomination hurdle, but zero out-of-pocket cost (tuition + R&B + medical + monthly stipend) and BS in engineering. Parallel track — doesn't foreclose the 3:2 portfolio if rejected. **Important caveat:** USNA does not accept transfer credit; if admitted after Hobart freshman year, Jack restarts as a plebe (Class of 2031 instead of 2030).

### Post-grad OCS as the Hobart-BA pivot (Option 2g)

If the dual-degree forks fail (Columbia / Dartmouth / WashU / RPI all reject) and Jack still wants an elite engineering credential, **post-grad OCS + GI Bill + Yellow Ribbon** is materially better than the Cal State BSME (Option 2f) for the final credential. Mechanism: Hobart BA (~$184K all-in) → 3–5 years of commissioned service → tuition-free MS at Dartmouth Thayer (uncapped Yellow Ribbon), Cornell, or Stanford. End state: Hobart BA + Navy/military service + elite engineering MS for the same $184K out-of-pocket as Option 2g (instead of $242K for 2f + a Cal State BSME). The trade is the service commitment itself, not the credential quality. Only the right play if Jack is independently drawn to military service. Full mechanism in [`hobart.md`](./hobart.md#how-option-2g-pays-for-the-engineering-credential); role-type detail in [`military.md`](./military.md).

### Pivot rules

- **GPA checkpoint end of sophomore year:** if cumulative < 3.25, have a serious conversation about direct transfer to an engineering school vs. another year of Hobart tuition.
- **Dartmouth 2:1:1:1 if pursued:** if visibly struggling by end of fall term at Dartmouth, talk to the Dual-Degree director immediately. Early pivot = more options intact.
- **If all 3:2 / 3:3 partners reject:** four forks. (a) Direct transfer Hobart → SDSU as a CA resident — cheapest at ~$196K all-in over 5 years, but no Hobart BA. (b) Finish Hobart BA + Cal State BSME (Option 2f, ~$242K / 6 yrs) — keeps the BA but adds ~$46K. (c) Finish Hobart BA + post-grad OCS + tuition-free MS (Option 2g, ~$184K + service) — cheapest *and* highest-credential, but requires the service commitment. (d) Finish Hobart BA + Syracuse ECS 4+2 MS (Option 2e, ~$289K / 6 yrs) — keeps things civilian and near-automatic at 3.2 GPA, but delivers an MS without an ABET BS, which is a weaker credential profile than (a) or (b). Ranking by credential quality: (c) > (b) ≈ (a) > (d). Pick (a) or (b) for ABET BS cleanliness, (c) if military service is independently attractive, (d) only if both (a)/(b) also fall through.

---

## 8. Action items & timeline

### Before enrolling at Hobart (spring/summer 2026)

1. **Confirm Dual-Degree liaison** (Prof. Donald Spector, `spector@hws.edu`) and get his sign-off on the 4-partner strategy.
2. **Pull pre-engineering course requirements for each 3-2 partner** and confirm Hobart offers equivalents. Columbia's ME prereqs are strictest; prepping for Columbia covers the others.
3. **Clarify with Hobart's registrar:** do Dartmouth/partner school grades transfer back into Hobart cumulative GPA, or only credits? (Affects how a bad partner-school year poisons fallback applications.)
4. **AP credit confirmation** — confirm with Hobart's Combined Plan liaison exactly which AP scores will yield credit, AND that Columbia will honor those credits.
5. **Rowing coach conversations** — Jack should contact:
   - **Chris Kerber** (Hobart head coach) about how 3-2 students handle eligibility and the Dad Vail transition.
   - **Nich Lee Parker** (Columbia lightweight coach) about precedent for 3-2 rowers, **Y5 graduate-eligibility workarounds (if any)**, and the realistic 1-year-at-Columbia LW expectation. Specific question: do any 3-2 students get an Ivy compliance waiver for Y5 rowing, or is the undergraduate-only rule absolute in practice?
   - **Trevor Michelson** (Dartmouth lightweight coach) about 2:1:1:1 LW rowing pattern with corrected eligibility picture (race Y3, return Hobart Y4 — openweight — or sit, then Y5 Dartmouth rowing ineligible as grad). What do 2:1:1:1 rowers typically do for Y4 and Y5?
   - **Mike Pearce** (UBC men's head coach) about lightweight rowing opportunities and potential Athletic Financial Awards.
6. **Revisit Colgate** — prior handoff noted Colgate has the same Columbia 3-2 pathway with currently stronger men's rowing (V8 finished 8th of 42 at HOCR 2024; 4th at 2024 IRA in varsity four). Worth a phone call to admissions to see if the door is still open.
7. **USNA application kickoff** — preliminary application, nomination requests (Senator, Rep, VP), Blue and Gold officer contact. Parallel track, low cost.

### Freshman year at Hobart (fall 2026 – spring 2027)

1. Take **Calc I, Calc II, calc-based Physics I with lab, introductory programming** in year 1. Prereqs for every 3-2 partner.
2. Maintain **3.5+ cumulative GPA.** Aim high — the threshold is the threshold.
3. Begin rowing at Hobart — freshman season counts as season 1 of 4 IRA eligibility.
4. By December of sophomore year, firm decision on Dartmouth 2-1-1-1 application (due Feb 1 of sophomore year).
5. **If pursuing USNA:** complete full candidate application, fitness assessment, medical, interview through fall 2026.

### Sophomore year

1. Complete Calc III, Physics II with lab, General Chemistry, remaining programming prereqs.
2. If applying to Dartmouth: submit by **Feb 1**. Decision mid-April.
3. **GPA checkpoint end of sophomore year:** if cumulative < 3.25, serious conversation about direct transfer to an engineering school.

### Junior year

1. Submit 3:2 portfolio applications: **Columbia (mid-Feb), RPI (rolling fall/spring), WashU (Feb 28).**
2. If Dartmouth 2:1:1:1 path active: Jack is at Dartmouth for year 3. Monitor performance carefully through fall term.
3. Decisions typically arrive by **June of junior year.**

### Senior year

1. If on Dartmouth path: submit BE application by **Dec 15.** Administrative for students in good standing.
2. Complete Hobart graduation requirements.

---

## 9. Open items to confirm

- [ ] **AP Physics C scores** — sat May 13–14, 2026; plan assumes 5/5 (scores post mid-July 2026). Revisit if either comes back below 4 — mainly affects WPI (Physics 1+2 not on WPI's chart, so Y1 would need PH 1110/PH 1120 inserted).
- [ ] **Hobart–WashU agreement structure** — 3:3 (BS+MS) or is a 3:2 (BS-only) option available?
- [ ] **Hobart's grade transfer policy** for Dartmouth/partner-school coursework (affects fallback applications).
- [ ] **Dartmouth 2:1:1:1 credit transfer back to Hobart** — sufficient to enable BA completion in senior year?
- [ ] **Colgate admissions status** — is the door still open? Stronger rowing than Hobart currently.
- [ ] **USNA nomination track** — start Senator/Representative nomination requests this spring.
- [ ] **UBC NUPOC eligibility** — confirm with Navy recruiter whether a UBC BASc qualifies as "regionally accredited U.S. school."
- [ ] **Jack's role-type preference** (technical/nuclear vs. operational line vs. engineering duty vs. SpecOps) — conversation, not research. Drives whether NUPOC, USNA, or post-grad OCS is the primary track.
- [ ] **Confirm Yellow Ribbon coverage** at Dartmouth Thayer, Cornell Engineering, and WashU McKelvey for graduate engineering (specific school-level participation and caps for the post-service MS path).
- [ ] **Ivy Y5 rowing eligibility in practice** — confirm with Columbia and Dartmouth compliance offices whether any 3:2 / 2:1:1:1 rowers have rowed in Y5 as graduate students, or whether the undergraduate-only rule is absolute. Drives whether the "1 year of Ivy LW" model is firm or has any exception path.

---

## 10. Source notes

- Prior research conducted April 2026 by another agent conversation (handoff documents since removed; all relevant content has been consolidated into this file).
- Supplementary research April 2026 this session:
  - USNA admissions policy on college transfers (USNA.edu Admissions FAQ).
  - Navy OCS engineering officer designator requirements (ABET-EAC requirement).
  - NUPOC eligibility for physics majors (navynupoc.com).
  - SDSU Mechanical Engineering change-of-major criteria (mechanical.sdsu.edu).
  - WSU Mechanical Engineering transfer requirements (mme.wsu.edu).
  - **IRA Eligibility & Operating Bylaws** (irarowing.com, 2020 revision) — adopts NCAA manual as baseline for men's heavy and lightweight rowing eligibility at member institutions.
  - **NCAA 4-season / 5-year baseline rules** (NCAA.com, CollegeNetWorth, 2adays NCAA eligibility guide) — relevant only as the IRA's adopted baseline, not as a direct governing body for Hobart/Columbia/Dartmouth men's rowing.
  - **Ivy League undergraduate-athlete policy** (Cornell Big Red Athletic Eligibility and Compliance 2024; Yale Bulldogs SA Handbook 2024; The Daily Pennsylvanian 2021 coverage of the one-time COVID graduate waiver and its non-renewal; The Harvard Crimson 2020 coverage of initial Ivy position; Athletic Business coverage of Cornell transfer portal consequences).
  - **Columbia 2025–26 men's lightweight rowing roster** (gocolumbialions.com) — confirms no graduate-student class designations currently on the roster.
  - **Columbia 2022 IRA All-Academic list** (gocolumbialions.com) — one GS-designated heavyweight rower reflects the 2021–22 one-time COVID waiver year, not a general exception.
- Exchange rate assumed $0.73 CAD/USD.
- All rankings, costs, and rowing results should be re-verified before any final commitment.
